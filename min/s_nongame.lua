...
if(count(g_menu)==0and g_game_mode_target==nil)draw_wavy_text("press 🅾️ to start!",28,96,1,1.4)
...
g_cred_scl+=.15g_cred_scl%=32end function draw_credits()for _x=-32+g_cred_scl,128,32do for _y=-g_cred_scl,128,32do spr(204,_x,_y,4,4)end end poke(24404,96)pal(g_pal_dark[1])sspr(0,58,128,70,0,58)sspr(0,7,128,18,0,7)sspr(39,38,52,20,39,38)pal()poke(24404,0)palt(4096)map(48,0,0,0,16,16)local _t={{"programming","bug fixes","general"},{"art, music","art, music","music"}}for _x=1,2do for _y=1,3do print_shd(_t[_x][_y],_x*64-46,_y*24+42,7,1)end end print_shd("director",58,42,7,1)print_shd("created by",44,1,7,1)print_shd("a plural system",34,26,7,1)print_shd("seven spirits, one body",18,32,7,1)end function update_intro()g_title_scroll+=.025g_title_scroll%=1if g_intro_countdown>0then g_intro_countdown-=1if(btnp()&48>0)g_intro_countdown=0
...
local _stages=count(g_levels[g_sss_menu_world])if(btnp(2))g_sss_menu_stage-=1
if(btnp(3))g_sss_menu_stage+=1
g_sss_menu_stage=mid(1,g_sss_menu_stage,_stages)if(btnp(4))set_game_mode(2,g_sss_menu_world,g_sss_menu_stage)
if(btnp(5))set_game_mode(0)
end function draw_stage_select()local _t,_y_offset=g_sss_colors,20+sin(g_sss_anim_factor>>3)*29for i=1,4do pal(_t[1][i],_t[g_sss_menu_world][i])end rectfill(0,0,127,127,1)map(0,14,-24+24*g_title_scroll,-2-_y_offset,19,3)map(0,17,-24*g_title_scroll,115+_y_offset,19,2)map(32,-2+g_sss_menu_world*2,0,1-_y_offset,16,2)local _bott=122+_y_offset if(g_sss_menu_world>1)?"⬅️previous area",1,_bott,7
if(g_sss_menu_world<count(g_levels))?"next area➡️",84,_bott,7
local _stages=count(g_levels[g_sss_menu_world])local _start=max(min(g_sss_menu_stage-5,_stages-9),0)local _sy1,_end,_show_timers,_is_hilite,_stagetime,_syt=23-_start*10,_start+10,setting_get(1)for _y,_st in ipairs(g_levels[g_sss_menu_world])do _is_hilite,_sy2=_y==g_sss_menu_stage,_sy1+10if _y>_start and _y<_end then _syt=_sy1+3local _stagetime=dget(_st.l_saveslot)if(_is_hilite)menu_draw_select(4,_sy1,90,_sy2)
pal(7,_is_hilite and 7or _t[g_sss_menu_world][2])?(_y>9and""or" ").._y.." ".._st.l_name,15,_syt,7
spr(_stagetime<=599.995and 241or 240,6,_sy1+1)if _show_timers then if(_stagetime<=_st.l_goaltime)?format_time(_stagetime),96,_syt,7
if(_stagetime<=_st.l_devtime)?"♥",120,_syt,7
end end _sy1=_sy2 end if g_sss_anim_factor<1then palt()if(g_sss_anim_incr<0)pal(1,_t[g_sss_menu_world_tgt][1])
