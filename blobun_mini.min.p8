...
f7,f9={51510,27795,14025,37740},{32769.5,42405.5,60855.5,-.5}f2,fe,n,T,U,fn,f8,_,o,d,fo,y=0,0,0,0,0,true,time(),0,1,{},false fu,fs,fa,I,fO,fh=J"0001115525511125",K("3c26915154",5),K("13b49a5d6",3),J(I),{64515,-925,-829,-733,-637,-541,-445,-248},{64140,63686,63880}fP()fQ()fR(1)fx()end function _update()fn=fn==false U+=.2U%=4f2+=.035f2%=1n+=.15n%=32T-=1if T<=0then T=14elseif T%7==1then L=true end fS()x=max(x-1)if(btnp()&48>0and x>45)x=45
if(count(d)>0and x>10)x=10
if(x<=45)_=min(_+.1,1)
if j~=nil then o=max(o-.1)if(o==0)local f={fT,fU,fV,fx,fW,fX}f[j+1](V,fm)_,o,j,V,fm=0,1
...
local o=d>e and r(c,d-1)or 0local e=l[o]or{unpack(n)}l[o]=e local f=e[a and 1or f"2"]i(e,f)i(n,f)t(c,d,f)end end end function er()local f={}f.obj_list,f.player,f.g_p_zap_turn,f.g_tile_count,f.playfield=fC(g),fC(g[1]),z,Q,et()add(E,f)if(count(E)>60)deli(E,1)
end function eb()local e=count(E)if(e==0)return
local f=E[e]if(e>0)deli(E)
g=f.obj_list g[1]=f.player z,Q=f.g_p_zap_turn,f.g_tile_count local e=g[1]e.nextdir,e.isdead,e.ismv,k,f1,L=-1,false,false,false,true,true e7(f.playfield)end function et()local f={}for e=0,34do f[e]={}for n=0,31do f[e][n]=mget(e,n)end end return f end function e7(n)for f=0,34do for e=0,31do mset(f,e,n[f][e])end end end function fC(e)local f={}for e,n in pairs(e)do f[e]=n end return f end function c(n,f,o,e)return tonum(sub(n,f,f+(e and e or 0)),o)end function K(n,f)local o,a,e={},#n\f for a=1,a do e=(a-1)*f o[a]=J(sub(n,e+1,e+f))end return o end function J(f)local e={}for n=1,#f do add(e,c(f,n,1))end return e end function p(f,e,n)return f+(e-f)*n end function v(n,o,a,d)for f=0,1do for e=0,1do mset(a+f,d+e,mget(n+f,o+e))end end end function R(o,a,f,e)local e,n,d,c,i,l,f=i.s_w<<1,i.s_h<<1,48+f\16<<1,f%16<<1,48+e\16<<1,e%16<<1for e=1,e,2do for n=1,n,2do f=mget(e+1,n)if f==o then v(i,l,e,n)elseif f==a then v(d,c,e,n)end end end end function S(f,e)local n=mget(f,e)if(n~=125and n~=127)return 0
v(126,24,f-1,e)return 1+S(f-2,e)+S(f+2,e)+S(f,e-2)+S(f,e+2)end function fl(e,n,o,a,d)local f=#e for f=1,f do n=print(sub(e,f,f),n,o+sin(f2+f/10)*d,a)end end function m(f,e,n,o,a)?f,e+1,n+1,a
return print(f,e,n,o)end function fD(f)return(f>=60and flr(f/60)..":"or"")..sub(flr(f%60)+100,2,3).."."..sub(flr(f%1*1000)+1000,2,4)end function en()local f=_*o==1fe+=.075fe%=1fd+=.015fd%=1if(x==45and fo==false)music(h(5)and 0or-1,0,7)fo=true
if x<=45and f then if(btn(6))poke(24368,1)
if((Z and l or btn(6))and count(d)==0)e9()
end O=mid(0,count(d)==0and(l or k)and O+.2or O-.2,1)if(f)for f in all(g)do f:onstep()end eu()
local f,e,n=g[1],i.s_w<<4,i.s_h<<4local o=f.inprt and cos(1-f.anim>>2)or f.anim X=i.s_w>=8and mid(0,(p(f.oldx,f.x,o)<<4)-48,e-112)or(e>>1)-56Y=i.s_h>7and mid(-8,(p(f.oldy,f.y,o)<<4)-48,n-108)or(n>>1)-60end function e8()poke(24405,0)if(fn)es()else eh()
if(L)ex()
if(f1)em()
ey()poke(24405,96)palt(0)local f,e=-32+n,-n if(q>=3)f,e=-32+-X/2,-32+-Y/2
for f=f,128,32do for e=e,128,32do spr(148,f,e,4,2)spr(152,f,e+16,4,2)end end poke(24404,96)pal()if(q<=2)for f=0,127do sspr(0,f,128,1,sin(fd+(f>>4))*1.4,f)end
poke(24404,0)camera(X,Y)local f,e=i.s_w*2+2,i.s_h*2+2palt(256)map(0,0,0,0,f,e,1)fE()map(0,0,-7,0,f,e,128)fE()for f in all(f5[f0])do f:ondraw()end palt(16384)map(0,0,0,0,f,e,12)palt()map(0,0,0,0,f,e,2)for f in all(g)do f:ondraw()end eg()camera(0,0)local o,a=h(1),count(d)local f,e=p(127,115,O),o and 0or 87poke(24404,96)pal(fu)if a>0then sspr(0,0,128,128,0,0)else sspr(e,0,128,9,e,0)if(O>0)sspr(0,f,128,16,0,f)
end pal()poke(24404,0)local e=B<=i.s_gt and 7or 13if(l and B<=i.s_dt)e=10
local n=dget(i.s_st)spr(n<599.995and 147or 146,88,0)if o then?fD(B),2,2,e
if(fj)fl("new time!",32,2,e,1.3)
if(n<=i.s_gt)?"⧗",81,2,9
if(n<=i.s_dt)?"♥",75,2,14
end local e=g[1].ttch rect(96,1,126,7,7)fillp(f7[ceil(U)])rectfill(124-max(1,e/Q*26),3,124,5,59)fillp()if(l or k)fl(l and"stage clear!"or"❎ undo",l and 42or 50,f+5,7,1.3)
if a==1then local f=sin(d[1].m_anim_factor>>2)*-32+-24local e=m("stage "..C,f,106,7,0)line(f-2,112,f+e,112,7)m(i.s_nm,f,115,7,0)m(i.s_at,f,122,13,0)elseif not l and q==1and C==1and e>5then m("🅾️ sprint",12,12,7,0)end end function es()local f="0c38010f00380438080f05380038010f0438113b0f01587f113c0f08113b587f0f0111432438010f00382538080f24380038010f2c3831430f01587f313b0f08313c587f0f01313b"for e=1,144,12do sspr(c(f,e,1,1),c(f,e+2,1,1),c(f,e+4,1,1),c(f,e+6,1,1),c(f,e+8,1,1),c(f,e+10,1,1))end line(88,127,104,127,0)line(0,56,0,71,0)end function eh()local a,e,f,n,o=f_,0,0,false,false for e=0,10do f,n,o=21+e,a&1<<e>0,a&8<<e>0sset(f,98,n and 7or 12)sset(f,96,o and 7or 0)sset(f,92,n and 9or 4)sset(f,90,a&32<<e>0and 10or 9)sset(f,88,o and 10or 0)end for e=0,4do f,n,o=8+e,a&16<<e>0,a&64<<e>0sset(f,98,n and 7or 12)sset(f,96,o and 7or 0)sset(f,90,n and 10or 9)sset(f,88,o and 10or 0)end for d=0,7do f,n,o=103-d,95-d,88+d e=a&1<<d>0and 7or 12sset(f,101,e)sset(50,103-d,e)sset(78,96+d,e)e=a&1<<d>0and 9or 4sset(f,91,e)sset(52,n,e)sset(76,o,e)e=a&4<<d>0and 9or 10sset(f,93,e)sset(50,n,e)sset(78,o,e)end f_>><=1end function ey()local o=fe spr(224,80,104,6,2)local f,n=80for a=0,32,16do clip(f,104,16,16)for e=0,15do n=sin(o+e/8)+104sspr(a+e,112,1,16,f+e,n)sspr(f,104+e,16,1,f,n+e)end f+=16end local e=g[1]clip()palt(0,false)spr(208,24,104,3,1)palt()if(e.haskey)spr(144,32,104)spr(144,39,104,1,1,true)
...
end function f:ondraw()local f,e=(self.x<<4)+8,self.y<<4if self.type<9then if self.oct_ind==nil or self.oct_ind==g[1].octg then local e,o,a,n=e+sin(self.anim)*2,self.spr%16<<3,self.spr\16<<3,ceil(sin(self.spin*.5)*-8)if(n<=3)rectfill(f+7,e+1,f+9,e+14,7)
...
Z=btn(4)and f6==false f6=btn(4)f.jgl=max(f.jgl-.2,0)if(btnp(5)and not l)eb()return nil
...
local o,a,d,c=f.inprt,16,f.x+cos(e>>2),f.y+sin(e>>2)if(f.oncnv or h(2))a=18
local n=mget((d<<1)+2,(c<<1)+1)if(n==121and f.pstate==1or n==51and f.haskey or fget(n)&a>0)o=true
f.nextdir,f.dir=-1,e if o and not k and not l then fq=true if(y==nil and stat(49)==-1and not f.oncnv)y=61956
if(not f.inprt and not f.oncnv)f.strnx,f.strny=f.x,f.y er()z+=1z%=3L=true
f.anim,f.sprint,f.ismv=0,tonum(h(3))~tonum(btn(4))==1,true if(f.inprt==false)f.oldx,f.oldy=f.x,f.y
if(e>=0)f.x,f.y=d,c
end end end if(l)f.dir=3
if(fq and not l)B=min(B+(time()-f8),599.999)
end function ek(f)f.ismv=false local o,a,d,c,l,h=f.x,f.y,(f.oldx<<1)+2,(f.oldy<<1)+1,true,true if(f.prvcrfl)v(96,18,d-1,c)Q-=S(d,c)f.ttch-=1y=62278f.prvcrfl=false
...
f.prvsltr=e==48for n=0,2do if(e==80+n)i=fa[n+1]f.pstate,y=n,fh[n+1]
end for n=0,7do if e==56+n then if f.octg==n then f.octg+=1y=fO[n+1]for e in all(g)do if(e.oct_ind==f.octg)ez(b,u+6,(e.x<<4)+12,(e.y<<4)+6)
...
break end end if f.inprt then f.inprt,i=false,fa[f.pstate+1]else for n=0,6,2do if(e==89+n)v(126,f.pstate<<1,r,t)f.oldx,f.oldy,f.inprt,y,f.x,f.y=o,a,true,63142,s.dst_x,s.dst_y
end end if(e==123and f.pstate==1)e=192v(112,14,r,t)
if(e>208)fG(f)h=false
if e\16==11and f.pstate==0or e\16==12or e==50or e==29and z==0or e==30and z==1or e==31and z==2then fG(f,true)elseif h then f.ttch+=1v(126,f.pstate<<1,r,t)end if(n~=nil)fH(b,u+8,J(n))
if(i~=nil)fI(b,u+12,i)
end function fG(f,e)k=true if(e)y=61448f.isdead=true fI((f.x<<4)+12,(f.y<<4)+12,fa[f.pstate+1])
...
if(o<1)a=f.oncnv and o or.5+cos(o*.5*sgn(o-.5))*-.5e,n=p(f.oldx<<4,f.x<<4,a),p(f.oldy<<4,f.y<<4,a)
e+=(-1+rnd(3))*f.jgl+8n+=(-1+rnd(3))*f.jgl+8local o,a=cos(f.dir>>2)<0,sin(f.dir>>2)<0local d,c=o and e or e-8,a and n or n-8if(f.dir%2==0)spr(211,d,n+7,3,1,o)spr(211,d,n,3,1,o,true)else spr(217,e+1,c,1,3,false,a)spr(217,e+8,c,1,3,true,a)
pal()end function fQ()cartdata("cs_blobun_"..el)if(dget(63)==0)dset(63,117)for f=0,59do dset(f,599.999)end dset(62,0x1.12)
end function h(f)return dget(63)&1<<f>0end function eF(f,n)local f,e=1<<f,dget(63)dset(63,n and e|f or e&~f)end function fk()return{stage=dget(62)<<4&15,world=dget(62)<<7&7}end function eo(f,e)dset(62,dget(62)&~.9922|f>>7|e>>4)end function f3(e)local f=0for e in all(b[e])do if(dget(e.s_st)<599.995)f+=1
end return f end function fJ(f)return f3(f)>=count(b[f])end function fA()local f=count(b)return f3(f)>=I[f]end function fr(f,e)for f in all(b[f])do if(dget(f.s_st)>(e and f.s_dt or f.s_gt))return false
end return true end function fc()for f=1,count(b)do if(not fJ(f))return false
end return true end function G(f)for e=1,count(b)do if(not fr(e,f))return false
...
palt()fillp(f9[flr(p(1,4,e))])rectfill(0,56,127,72,12)fillp()end function fz()if(_*o==1and btnp()&48>0)s(0)
...
end end function ef()A=mid(0,A+D,1)u+=.02u%=1local f=count(b)if A==1then if(btnp(0)and r>1)y=61700N=max(r-1,1)D=-.125
if(btnp(1)and r<count(b)and f3(r)>=I[r])y=61700N=min(r+1,f)D=-.125
end if(A==0)D=.125r=N N=nil
...
end end function e2()local o,f=ee,20+sin(A>>3)*29for f=1,4do pal(o[1][f],o[r][f])end rectfill(0,0,127,127,1)map(0,14,-24+24*u,-2-f,19,3)map(0,17,-24*u,115+f,19,2)map(32,-2+r*2,0,1-f,16,2)local f,e=122+f,I[r]if(r>1)?"⬅️previous area",1,f,7
local n=f3(r)?n>=e and(r<count(b)and"next area➡️"or"")or"▶solve "..e-n,84,f,7
local e=count(b[r])local a=max(min(w-5,e-9),-1)local n,d,i,c,l,f=13-a*10,a+10,h(1)for e=0,e do c,fK=e==w,n+10if e>a and e<d then f=n+3local a=b[r][max(1,e)]local d=dget(a.s_st)if(c)fL(4,n,90,fK)
pal(7,c and 7or o[r][2])if e>0then?(e>9and""or" ")..e.." "..a.s_nm,14,f,7
spr(d<=599.995and 241or 240,6,n+1)if i then if(d<=a.s_gt)?fD(d),96,f,7
if(d<=a.s_dt)?"♥",120,f,7
end else?"back",26,f,7
pal(7,10)if(fJ(r))spr(241,6,n+1)
if i then if(fr(r,false))?"⧗",113,f,7
if(fr(r,true))?"♥",120,f,7
end end end n=fK end if A<1then palt()if(D<0)pal(1,o[N][1])
fillp(f9[flr(p(1,4,A))])if(D>0)rectfill(0,22,127,112,1)else rectfill(0,0,127,127,1)
...
local e,b,o=count(f.m_items),count(g)>0and 147or 241,p(5,f.m_h,f.m_anim_factor)local n,e,a=f.m_x,f.m_y-(o>>1)local o,d,i,l,r,t=n+f.m_w,e+o rectfill(n,e,o,d,1)rect(n+1,e+1,o-1,d-1,13)local u,e=n+8,e+6for s,c in pairs(f.m_items)do if e<d-9then i,l,r,t=n+3,e-3,o-3,e+7a=s==f.m_highlight if(a)fL(i,l,r,t)
...
end)f.i_setting=e return f end function e9()Z,f6,y=false,false,61828local f if(not ea and fA())f=1
if(not ed and fc())f=2
if(not e6 and G(false))f=3
if(not e5 and G(true))f=4
if(f~=nil)s(5,f)return
local e,n,o,f=a((l and"next"or"skip").." puzzle",function()if(C+1>count(b[q]))s(1)else s(2,q,C+1)
end),a("restart puzzle",function()s(2,q,C,true)end),a("stage select",function()s(1)end)if(l)f={e,n,o}else f={a("back",fM),n,e,a((f0==2and"hide"or"show").." hints",function()f0=f0==2and 1or 2fy()end),o,a("options",fN),a("go to title",function()s(0)end)}
ft(16,64,96,f)end function e_()ft(24,96,80,{a("start game",function()s(1)end),a("continue",function()local f=fk()s(2,f.world,f.stage)end),a("options",fN),a("credits",function()s(4)end),a("back",function()s(3)end)})end function fN()ft(16,count(g)>0and 64or 72,96,{a("back",fM),f("show timers",1),f("slime overlap",2),f("sprint by default",3),f("sound effects",4),f("music",5),f("lesbians allowed",6),a("exit game",function()extcmd"shutdown"end),a("pico8 menu",ej)})end function ej()extcmd"pause"end function fL(f,e,n,o)rectfill(f,e,n,o,2)fillp(f7[ceil(U)])rect(f,e,n,o,154)fillp(0)end function f4(e,n,o,a,d,f)if(count(P)<100)add(P,{x=e,y=n,spd=o,dir=a,col=d,time_max=f,time=f})
end function fI(d,c,a)local f,e,n for o=0,.9,.1do f,e,n=d+cos(o)*4,c+sin(o)*4,o*360f4(f,e,.9,n,rnd(a),12)f4(f,e,1.8,n,rnd(a),8)end end function ez(o,a,e,n)for f=0,.8,.2do f4(p(o,e,f),p(a,n,f),0,0,14,p(5,12,f))end fH(e,n,{14,7})end function fH(e,n,o)for f=0,.9,.1do f4(e+cos(f)*4,n+sin(f)*4,.8,f*360,rnd(o),8)end end function eu()for f in all(P)do local e=f.dir/360f.x+=f.spd*cos(e)f.y+=f.spd*sin(e)f.time-=1if(f.time<=0)del(P,f)
