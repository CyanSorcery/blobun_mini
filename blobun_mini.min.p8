...
local f,e,n=g[1],i.s_width<<4,i.s_height<<4local o=f.inportal and cos(1-f.anim>>2)or f.anim X=i.s_width>=8and mid(0,(p(f.oldx,f.x,o)<<4)-48,e-112)or(e>>1)-56Y=i.s_height>7and mid(-8,(p(f.oldy,f.y,o)<<4)-48,n-108)or(n>>1)-60end function e8()poke(24405,0)if(fn)es()else eh()
if(L)ex()
if(f1)em()
...
Z=btn(4)and f6==false f6=btn(4)f.jiggle=max(f.jiggle-.2,0)if(btnp(5)and not l)eb()return nil
f.anim=min(f.anim+((f.sprint or f.onconvey or f.inportal)and.2or.1111),1)if f.anim>=.65then local n=J"2013"for e=0,3do if(btn(e))f.nextdir=n[e+1]
end end if f.anim+.001>=1and not l and not k then if(f.ismove)ek(f)
if l==false and f.tilestouched>=Q then l,y=true,62483if(B<dget(i.s_saveslot))dset(i.s_saveslot,B)fj=true
end if(Z)f.jiggle=1
...
f.nextdir,f.dir=-1,e if o and not k and not l then fq=true if(y==nil and stat(49)==-1and not f.onconvey)y=61956
if(not f.inportal and not f.onconvey)f.startturnx,f.startturny=f.x,f.y er()z+=1z%=3L=true
...
end function ek(f)f.ismove=false local o,a,d,c,l,h=f.x,f.y,(f.oldx<<1)+2,(f.oldy<<1)+1,true,true if(f.prevcrackedfloor)v(96,18,d-1,c)Q-=S(d,c)f.tilestouched-=1y=62278f.prevcrackedfloor=false
...
f1=true end if(f.prevslimetrap)v(126,26,d-1,c)y=62084
f.prevslimetrap=e==48for n=0,2do if(e==80+n)i=fa[n+1]f.pstate,y=n,fh[n+1]
...
end else l=false end end end if(f.octogems==8)R(27,28,74,106)f.octogems=0
...
break end end if f.inportal then f.inportal,i=false,fa[f.pstate+1]else for n=0,6,2do if(e==89+n)v(126,f.pstate<<1,r,t)f.oldx,f.oldy,f.inportal,y,f.x,f.y=o,a,true,63142,s.dst_x,s.dst_y
end end if(e==123and f.pstate==1)e=192v(112,14,r,t)
if(e>208)fG(f)h=false
if e\16==11and f.pstate==0or e\16==12or e==50or e==29and z==0or e==30and z==1or e==31and z==2then fG(f,true)elseif h then f.tilestouched+=1v(126,f.pstate<<1,r,t)end if(n~=nil)fH(b,u+8,J(n))
if(i~=nil)fI(b,u+12,i)
end function fG(f,e)k=true if(e)y=61448f.isdead=true fI((f.x<<4)+12,(f.y<<4)+12,fa[f.pstate+1])
end function ev(f)if(f.isdead or f.inportal or not h(6))return
local d,e,n,o,a=f.dir,f.x<<4,f.y<<4,f.anim if(f.pstate==1)pal(3,8)pal(11,9)
if(f.pstate==2)pal(3,13)pal(11,6)
if(o<1)a=f.onconvey and o or.5+cos(o*.5*sgn(o-.5))*-.5e,n=p(f.oldx<<4,f.x<<4,a),p(f.oldy<<4,f.y<<4,a)
...
end function h(f)return dget(63)&1<<f>0end function eF(f,n)local f,e=1<<f,dget(63)dset(63,n and e|f or e&~f)end function fk()return{stage=dget(62)<<4&15,world=dget(62)<<7&7}end function eo(f,e)dset(62,dget(62)&~.9922|f>>7|e>>4)end function f3(e)local f=0for e in all(b[e])do if(dget(e.s_saveslot)<599.995)f+=1
end return f end function fJ(f)return f3(f)>=count(b[f])end function fA()local f=count(b)return f3(f)>=I[f]end function fr(f,e)for f in all(b[f])do if(dget(f.s_saveslot)>(e and f.s_devtime or f.s_goaltime))return false
