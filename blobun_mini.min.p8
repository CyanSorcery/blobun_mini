...
return print(f,e,n,a)end function fD(f)return(f>=60and flr(f/60)..":"or"")..sub(flr(f%60)+100,2,3).."."..sub(flr(f%1*1000)+1000,2,4)end function ee()local f=_*a==1f8+=.075f8%=1fa+=.015fa%=1if(x==45and f5==false)music(h(5)and 0or-1,0,7)f5=true
if x<=45and f then if(btn(6))poke(24368,1)
if((Z and l or btn(6))and count(d)==0)et()
...
if(L)ex()
if(f1)ey()
em()poke(24405,96)palt(0)local f,e=-32+n,-n if(q>=3)f,e=-32+-X/2,-32+-Y/2
for f=f,128,32do for e=e,128,32do spr(148,f,e,4,2)spr(152,f,e+16,4,2)end end poke(24404,96)pal()if(q<=2)for f=0,127do sspr(0,f,128,1,sin(fa+(f>>4))*1.4,f)end
...
if(n<=i.s_gt)?"⧗",81,2,9
if(n<=i.s_dt)?"♥",75,2,14
end local e=g[1].ttch rect(96,1,126,7,7)fillp(f7[ceil(U)])rectfill(124-max(1,e/Q*26),3,124,5,59)fillp()if(l or v)fl(l and"stage clear!"or"❎ undo",l and 42or 50,f+5,7,1.3)
if o==1then local f=sin(d[1].m_anim_factor>>2)*-32+-24local e=y("stage "..C,f,106,7,0)line(f-2,112,f+e,112,7)y(i.s_nm,f,115,7,0)y(i.s_at,f,122,13,0)elseif not l and q==1and C==1and e>5then y("🅾️ sprint",12,12,7,0)end end function es()local f="0c38010f00380438080f05380038010f0438113b0f01587f113c0f08113b587f0f0111432438010f00382538080f24380038010f2c3831430f01587f313b0f08313c587f0f01313b"for e=1,144,12do sspr(c(f,e,1,1),c(f,e+2,1,1),c(f,e+4,1,1),c(f,e+6,1,1),c(f,e+8,1,1),c(f,e+10,1,1))end line(88,127,104,127,0)line(0,56,0,71,0)end function eh()local o,e,f,n,a=f_,0,0,false,false for e=0,10do f,n,a=21+e,o&1<<e>0,o&8<<e>0sset(f,98,n and 7or 12)sset(f,96,a and 7or 0)sset(f,92,n and 9or 4)sset(f,90,o&32<<e>0and 10or 9)sset(f,88,a and 10or 0)end for e=0,4do f,n,a=8+e,o&16<<e>0,o&64<<e>0sset(f,98,n and 7or 12)sset(f,96,a and 7or 0)sset(f,90,n and 10or 9)sset(f,88,a and 10or 0)end for d=0,7do f,n,a=103-d,95-d,88+d e=o&1<<d>0and 7or 12sset(f,101,e)sset(50,103-d,e)sset(78,96+d,e)e=o&1<<d>0and 9or 4sset(f,91,e)sset(52,n,e)sset(76,a,e)e=o&4<<d>0and 9or 10sset(f,93,e)sset(50,n,e)sset(78,a,e)end f_>><=1end function em()local a=f8 spr(224,80,104,6,2)local f,n=80for o=0,32,16do clip(f,104,16,16)for e=0,15do n=sin(a+e/8)+104sspr(o+e,112,1,16,f+e,n)sspr(f,104+e,16,1,f,n+e)end f+=16end local e=g[1]clip()palt(0,false)spr(208,24,104,3,1)palt()if(e.haskey)spr(144,32,104)spr(144,39,104,1,1,true)
...
Z=btn(4)and fo==false fo=btn(4)f.jgl=max(f.jgl-.2,0)if(btnp(5)and not l)er()return nil
...
end end if f.anim+.001>=1and not l and not v then if(f.ismv)ev(f)
if l==false and f.ttch>=Q then l,m=true,62483if(B<dget(i.s_st))dset(i.s_st,B)fj=true
end if(Z)f.jgl=1
...
end function ev(f)f.ismv=false local a,o,d,c,l,h=f.x,f.y,(f.oldx<<1)+2,(f.oldy<<1)+1,true,true if(f.prvcrfl)k(96,18,d-1,c)Q-=S(d,c)f.ttch-=1m=62278f.prvcrfl=false
...
f.prvsltr=e==48for n=0,2do if(e==80+n)i=f6[n+1]f.pstate,m=n,fh[n+1]
end for n=0,7do if e==56+n then if f.octg==n then f.octg+=1m=fO[n+1]for e in all(g)do if(e.oct_ind==f.octg)ez(t,u+6,(e.x<<4)+12,(e.y<<4)+6)
...
if e\16==11and f.pstate==0or e\16==12or e==50or e==29and z==0or e==30and z==1or e==31and z==2then fG(f,true)elseif h then f.ttch+=1k(126,f.pstate<<1,b,r)end if(n~=nil)fH(t,u+8,J(n))
if(i~=nil)fI(t,u+12,i)
end function fG(f,e)v=true if(e)m=61448f.isdead=true fI((f.x<<4)+12,(f.y<<4)+12,f6[f.pstate+1])
end function ek(f)if(f.isdead or f.inprt or not h(6))return
...
end function h(f)return dget(63)&1<<f>0end function eF(f,n)local f,e=1<<f,dget(63)dset(63,n and e|f or e&~f)end function fv()return{stage=dget(62)<<4&15,world=dget(62)<<7&7}end function e5(f,e)dset(62,dget(62)&~.9922|f>>7|e>>4)end function f3(e)local f=0for e in all(r[e])do if(dget(e.s_st)<599.995)f+=1
...
palt()fillp(ft[flr(p(1,4,e))])rectfill(0,56,127,72,12)fillp()end function fz()if(_*a==1and btnp()&48>0)s(0)
...
pal()poke(24404,0)palt(4096)if(M==nil)map(48,0,0,0,16,16)local n={{"programming","bug fixes","general"},{"art, music","art, music","music"}}for f=1,2do for e=1,3do y(n[f][e],f*64-46,e*24+42,7,1)end end y("director",58,42,7,1)y("created by",44,1,7,1)y("a plural system",34,26,7,1)y("seven spirits, one body",18,32,7,1)else map(32,8+M*2,0,32,16,2)local f={[[ you've overcome many
...
for you. you're too good
  at the game! not many
   people can do this.
    ...
    you are amazing!]]}y(f[M],16,56,7,0)
...
if(btnp()&48>0or f)m=61828e_()
end end function ef()A=mid(0,A+D,1)u+=.02u%=1local f=count(r)if A==1then if(btnp(0)and b>1)m=61700N=max(b-1,1)D=-.125
if(btnp(1)and b<count(r)and f3(b)>=I[b])m=61700N=min(b+1,f)D=-.125
...
if(w<0)w=f
if(w>f)w=0
if btnp()&48>0or e then if(w==0)s(0)m=63333else s(2,b,w)music(-1,500)m=61828
...
fillp(ft[flr(p(1,4,A))])if(D>0)rectfill(0,22,127,112,1)else rectfill(0,0,127,127,1)
...
end e+=10end pal()end})end function fm()for f in all(d)do f.m_anim_incr=-.25end end function fM()m=63333d[count(d)].m_anim_incr=-.25end function o(f,e)return{i_label=f,i_onclick=e,i_setting=nil}end function f(f,e)local f=o(f,function(f)eF(f.i_setting,h(f.i_setting)==false)if(f.i_setting==5)music(h(5)and 0or-1,0,7)
end)f.i_setting=e return f end function et()Z,fo,m=false,false,61828local f if(not e6 and fA())f=1
if(not ea and fc())f=2
if(not eo and G(false))f=3
