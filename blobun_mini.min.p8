...
end O=mid(0,count(d)==0and(l or v)and O+.2or O-.2,1)if(f)for f in all(g)do f:onstep()end eh()
...
local e,n,f=(btn(4)or e.sprint)and e.oncnv==false,e.blink<4or l f=e and 214or 215if(n or v)f=216
...
end end if f.anim+.001>=1and not l and not v then if(f.ismv)eF(f)
if not l and f.ttch>=Q and not v then l,m=true,62483if(B<dget(i.s_st))dset(i.s_st,min(599,B))fA=true
end if(Z)f.jgl=1
...
f.nextdir,f.dir=-1,e if a and not v and not l then fq=true if(m==nil and stat(49)==-1and not f.oncnv)m=61956
...
palt()fillp(fu[flr(p(1,4,e))])rectfill(0,56,127,72,12)fillp()end function fF()if(j*a==1and btnp()&48>0)if D~=nil then u(1)else u(0)end
...
local e,n,a,f=o((l and"next"or"skip").." puzzle",function()if(C+1>count(r[z]))u(1)else u(2,z,C+1)
end),o("restart puzzle",function()u(2,z,C,true)end),o("stage select",function()u(1)end)if(l)f={e,n,a}else f={o("back",fN),n,e,o((f0==2and"hide"or"show").." hints",function()f0=f0==2and 1or 2fg()end),a,o("options",fO),o("go to title",function()u(0)end)}
fr(16,64,96,f)end function eA()fr(24,96,80,{o("start game",function()u(1)end),o("continue",function()local f=fz()u(2,f.world,f.stage)music(-1,500)end),o("options",fO),o("credits",function()u(4)end),o("back",function()u(3)end)})end function fO()fr(16,count(g)>0and 64or 72,96,{o("back",fN),f("show timers",1),f("slime overlap",2),f("sprint by default",3),f("sound effects",4),f("music",5),f("lesbians allowed",6),o("exit game",function()extcmd"shutdown"end),o("pico8 menu",eq)})end function eq()extcmd"pause"end function fM(f,e,n,a)rectfill(f,e,n,a,2)fillp(f7[ceil(U)])rect(f,e,n,a,154)fillp(0)end function f4(e,n,a,o,d,f)if(count(P)<100)add(P,{x=e,y=n,spd=a,dir=o,col=d,time_max=f,time=f})
