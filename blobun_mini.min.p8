pico-8 cartridge // http://www.pico-8.com
version 42
__lua__
function _init()cls(1)?"loading...",1,122,13
flip()memcpy(32768,0,17152)_={{},{},{},{},{},{}}ff,ft=0,0local e,f=32768,%32768while(f~=0)add(_[f>>13&7],e+2)e+=2+(f&8191)f=%e
f7,fu={51510,27795,14025,37740},{32769.5,42405.5,60855.5,-.5}f2,f8,n,T,U,fe,fn,j,a,d,f5,m=0,0,0,0,0,true,time(),0,1,{},false fs,fh,f6,J,fP,fx=K"0001115525511125",L("3c26915154",5),L("13b49a5d6",3),K(J),{64515,-925,-829,-733,-637,-541,-445,-248},{64140,63686,63880}fQ()fR()fS(1)fy()end function _update()fe=fe==false U+=.2U%=4f2+=.035f2%=1n+=.15n%=32T-=1if T<=0then T=14elseif T%7==1then M=true end fT()x=max(x-1)if(btnp()&48>0and x>45)x=45
if(count(d)>0and x>10)x=10
if(x<=45)j=min(j+.1,1)
if q~=nil then a=max(a-.1)if(a==0)local f={fU,fV,fW,fy,fX,fY}f[q+1](V,fm)j,a,q,V,fm=0,1
end for f,e in pairs(d)do e:m_step(f)end if(not h(4))m=nil
if(m~=nil)sfx(m>>10&63,3,m>>5&31,m&31)m=nil
fn=time()if(fn<0)stop"game has been running for too long. take a break!"
end function _draw()fZ()for f,e in pairs(d)do e:m_draw(f)end local f=q==2or q==nil and count(g)>0local e=f and V~=nil and V or k or 1local n,f,e=f and fh[1][e]or 1,fh[2][e],64*cos(j*a>>2)if(e>0)rectfill(0,0,e,127,n)rectfill(127-e,0,127,127,n)
if x>0then local d,a="stage "..C,cos(abs(max(abs(x-45),35)-35)/40)*64-64local n,o,a=a+8,a+9,y(d,a+12,97,7,f)line(o,105,o+a,105,f)line(n,104,n+a,104,7)y(i.s_nm,n,107,7,f)if(e>0)?i.s_at,n,115,f
end end function u(e,f,n,a)if(e==5)D=f f=nil
q,V,fm,e1,x=e,f,n,a,0fg()if(q~=2)music(-1,500)f5=false
end function fp(f,e)fT,fZ,x,g=f,e,0,{}pal()end function e(f,e,n)fw(6)fv(1)fk(1,0,0)music(h(5)and f or-1,0,7)s=0fp(e,n)end function fy()e(-1,e0,e3)W=90end function fU()e(0,e4,ef)end function fV()e(4,e2,e8)local f=fz()b,N=mid(1,f.world,count(r))w=mid(1,f.stage,count(r[b]))s,A,E,ee=0,0,.125,L("13b224cd02415d6d2494",4)end function fX()e(4,fF,f_)D=nil end function fY()e(4,fF,f_)end function fW(f,e)fp(en,e5)local n=ff~=2f=mid(1,f,count(r))e=mid(1,e,count(r[f]))k,C,i=f,e,r[f][e]e6(f,e)fa,fj,X,Y=0,61936.8827,0,0g,z,l,Z,fo,F,M,f1,O,fq,x,B,fA,P,G,fd,f0,Q,ea,eo,ed,ec,ei={},false,false,false,false,0,true,true,0,false,e1==true and 0or 90,0,false,{},{},{{},{}},1,i.s_tl,fB(),fc(),H(false),H(true),fi(f)fv(2)fw(f)if(n)poke(24405,0)local e=L("2854ef234924d6d54924",4)for n=1,4do pal(e[1][n],e[f][n])end spr(1,8,0,14,1)spr(16,0,8,2,2)poke(24405,96)pal()el(f)fk(2,96,0)
local e,n,f=i.s_d,i.s_w+2,i.s_o_c-1for f=1,#e,2do local e=c(e,f,1,1)v(96+e\16*2,e%16*2,(f-1)\2%n*2-1,(f-1)\2\n*2-1)end for f=0,f do local f=eb(sub(i.s_o_s,f*5+1,f*5+6))if(f.type==11)add(fd[1],fC(f.spr,f.x,f.y))else add(g,f)
end for f in all(i.s_hn)do add(fd[2],fC(f))end end function fC(f,e,n)local f={dir=type(f)=="number"and f or c(f,1,1),x=((e and e or c(f,2,1))<<4)+12,y=((n and n or c(f,3,1))<<4)+12}function f:ondraw()fl=self.dir spr(fl%2==0and 254or 255,self.x,self.y,1,1,cos(fl>>2)<0,sin(fl>>2)>=0)end return f end function fQ()local n,d,e,f,a,o=r r={}for n in all(n)do d={}for n in all(n)do e={}f=2a=c(n,1,1)e.s_nm=sub(n,f,f+a)f+=a+2a=c(n,f-1,1)e.s_at=sub(n,f,f+a)f+=a+1e.s_w=c(n,f,1)+1e.s_st=c(n,f+1,0,1)e.s_gt=c(n,f+3,0,7)e.s_dt=c(n,f+11,0,7)f+=19a=c(n,f,1)*3e.s_hn={}for a=1,a,3do add(e.s_hn,sub(n,f+a,f+a+2))end f+=a+1a=c(n,f,1,1)o=f+2+a*5e.s_o_c=a e.s_o_s=sub(n,f+2,o-1)e.s_tl=c(n,o,1,1)e.s_d=sub(n,o+2)e.s_h=#e.s_d\2\(e.s_w+2)-2add(d,e)end add(r,d)end end e9="4fe5729bd9f69189773150d1dfc52318"J="88774"r={{"6tadpole3ashe601004.8344002.8344c3010023120130231331321310300403500510600100b3101b5100b0200b1203b3301124e4e4e38181818404e38184033ff01ff13403321131d010b01ff3b3301ff0cff01ff013b4c3101ff01ff01394d4e4c41414141414d4e","bplaying card4riley302004.8344002.851160000100203302313210100000123818181818403321ff01ff3b33ff19ff013b33010c01ff3b33ff01ff013b3301ff01ff3b4c414141414d","6big can3ashe542009.1010006.584480100200303400413512522420101000284e3818181818404e38102101ff01134033ff01ff01ff013b33011901ff01ff3b33ff1aff01ff013b33011a01ff01ff3b33ff0cff01ff013b3301ff01ff01ff3b4c3101ff01ff394d4e4c414141414d4e","acredit card4riley657006.5011004.501173023030040141241232220500200a1300a0013a5360a60531c3818181818181818403345ff01ff01ff053b36141414141414143f3321ff01ff01ff013b33ff45ff01ff05ff3b3301ff01ff01ff013b4c414141414141414d","efractured floor3ashe605006.8344004.836160450551651642633530504500a1160a6011a2540a40251e3818181830181818403301ff011a25ff053b33ff05ff121b01ff3b3301ff01ff1214143f3614141b01ff01ff3b3301ff121b01ff013b33ff01251a2101ff3b4c4141414b4141414d","btwisted path3ashe504005.3344003.334431031020010100300154e3818181818404e3810ff01ff01134033ff01ff19ff013b33010b011f0dff3b332101890cff013b4c413101ff01394d4e4e4c4141414d4e","asilly swine3ashe603007.7010005.501141051041031020100500223818181830181818403301ff011a01ff013b33ff0bff0cff0bff3b3301ff89ff89ff013b33ff11142a140dff3b3301ff010c01ff013b332101ff01ff01ff3b4c414141414141414d","9mini robot4riley558005.5011003.517743200211310300302000a4010a1040164e3818181818404e4e332521ff253b4e381001ff01ff1340330189898989ff3b4c3101ff01ff394d4e4c414141414d4e","ccompact pipes5autumn450004.6677002.767730021122110500200a2220a2022a4200a004211381818181818403325ff65ff013b33ff89ff89ff3b3321ff65ff253b4c41414141414d","dspiral portals4riley634006.8344004.867762331230220323423430303300a5414a14541e38181818181818184033018901ff0189013b33ff0111140d01ff3b33011901ff0119013b33ff0cff21ff0cff3b33010501ff0105013b4c414141414141414d","cstylish bunny5autumn632007.4677005.501171560551652641542531430305600a4020a2040214e4e38183018404e4e4e3810451a4513404e4e3301ff1aff013b4e4e33ff010c01ff3b4e381001ff01ff0113403301ff0bff0bff013b33ff01ff89ff01ff3b4c31ff01ff0121394d4e4c41414141414d4e","calien shuttle4riley656006.5011004.501140340441540530303400a4222a22421c4e381818181818404e3810ff01ff01ff134033ff89ff19ff89ff3b33010b051a050b013b33ff01ff0cff01ff3b4c31ff012101ff394d4e4c41414141414d4e","dtricky portals3coco411005.3344003.567770321421412403302312210503200a1220a2012a2322a222315381818181818403301ff25ff013b33ff018901ff3b3301256521013b33ff016501ff3b4c41414141414d","bsaving space5autumn549008.1677005.901031030021120100300244e4e381818404e4e4e381001ff13404e3810018989ff1340330189898989ff3b332189898989013b4c31ff898901394d4e4c31ff01394d4e4e4e4c41414d4e4e"},{"cbarrier break3ashe406005.6677003.66772001111020010032155174e3818181818403810444443013b3321444243ff3b3301ff0143013b334301ff43ff3b4c31434343013b4e4c414141414d","estaggered coins3coco627005.8344003.834421021010400200400564315646156183818181818181818403362ff01ff01ff013b33ff0bff62ff0b623b3321ff010b0164013b4c3101ff01ff01633b4e4c4141414141414d","dpath of hearts4riley459005.1677003.1677330200311305002001015314153a3212a123214381818181818403304040404043b3302010301023b3321650165013b3304010401043b4c41414141414d","btoggled tris3mika536005.8344003.83446003013123222212102040030035055a5300a00531838181818181818403365ff01ff01423b33ff44444343013b330144444343ff3b332101ff01ff653b4c4141414141414d","cmasked hearts4riley535006.1677004.317781031021010000103203212220200300112531a381818181818184033040401ff03033b33040bff010b033b33010201ff01ff3b332101ff01ff013b4c31ff030301394d4e4c414141414d4e","cdiamond coins3ashe607007.9344005.68446024034044054164163050240046056402564325620454233818181818181818403301ff01ff01ff623b33ff6464016363ff3b33626464626363243b33ff01ff23ff01243b33222321ff2424013b4c414141414141414d","9swituation5autumn451005.1677003.16772322023050220010053340553125513353143818181818184033024301ff423b33ff010444443b3301422104033b3304430402ff3b4c41414141414d","ekey to diamonds3ashe637006.1100004.5100510310210100001003003009209f25254193818181830181818403301ff2c1a2323233b33ff01ff0c2301233b3301ff018a2422243b332101ff192424243b4c4141414b4141414d","dtriangle coins3coco626006.5011004.50116163162261251241331060630045056412563425530355433561c3818181818181818403301ff01444462633b33ff01ff01ff01ff3b33646243ff4243013b334264ff62ff43213b4c414141414141414d","dunlocking path3ashe608006.3344004.334472232131031021010000100502300a4121a21419039f363551b3818181818181818403301ff448a44ff013b33ff01050b0501ff3b3301ff014301ff013b332c012143ff01423b4c414141414141414d","btricky trail4riley700008.8010006.50113104103002070040013253442561235345356224544645628381818181818181818403301ff01ff01ff01ff3b33ff640423ff0103013b3301642302626401ff3b33ff6302016462ff013b332103226301ff62ff3b4c41414141414141414d","atoggle maze3coco612009.8010007.23446263253243233223213090630042056461562125436455a0522a2205425569359f2b3818181818181818403344ff626301ff433b33ff01ff63ff01623b33012205238a23013b33242424448944213b3323ff01ff43ff423b330501622cff01ff3b4c414141414141414d","dfractured isle5willow620008.8677006.35104266256246236090660035055a6100a00612035431355a0664a640621654273818181818181818403325ff01444442013b33ff01ff444401253b36141414141b24243b332242ff011a24243b33432323441aff053b33432323441214143f33052201ff01ff213b4c414141414141414d","cnumerous keys4riley633009.8010007.117752432332232131030a043003405540156441569629f113534045624454a0560a60052b381818181818181840330303014442ff053b336201ff016264643b330144444343242c3b33ff0204012124243b33626363632289013b330501ff018a23233b4c414141414141414d"},{"ewatch your step5autumn330004.3011002.501141030021122110200300310551038181818184033014201ff3b33474443013b3301ff43ff3b332101ff273b4c414141414d","ccareful moves5willow518005.1677003.16774143142241231030430035055223541438181818181818403301ff01ff27423b33ff23ff0124433b4c3123014701394d4e33232227213b4e4e4c414141414d4e","cdiamond house3ashe509006.1677004.18442154153030540020254252541a4e4e381818404e4e381810232313184033ff012323ff013b332224242424223b33ff47ff0147013b33010701ff07213b4c4141414141414d","dforbidden path5willow439006.1100004.1100512202103134124202022002445419381818181818403323ff01ff233b33ff014701ff3b3301272107013b33ff010701ff3b3307ff01ff223b4c41414141414d","fkey in the woods5willow440006.1100004.5100524413403314324202044009049f19381818181818403366ff01ff463b33ff01ff01ff3b3301ff01ff013b33ff018a01ff3b332c0601ff213b4c41414141414d","9conveyance3roxy528006.8344004.83445104103102101000030040011353253541e38181818181818403301ff66ff01663b33ff664601ff663b330166060601663b33ff02030123223b33210303ff23233b4c4141414141414d","dunstable floor4riley417005.8344003.851040040140240340200400321551a381818181818403301ff0144443b33ff014244443b334343fe09fe3b334343ff89ff3b3321ff0109013b4c41414141414d","atiny cracks5autumn352004.3344002.3344201302304013009319fa0330a300310381818181840338aff47253b330966092c3b330147feff3b332521fffe3b4c414141414d","cdigging holes4riley638005.5011003.50113031141240030310033055333551c3818181818181818403343ff014201ff443b33fffe092109feff3b330109fefffe09013b334301ff42ff01443b4c414141414141414d","aforest maze5autumn531006.4677004.884441040033130140600400a5100a00519029f152539549f1f3818181818181840336589464646273b338a03ff8aff653b332cff010926023b33ff0404feff013b3321ff4709012c3b4c4141414141414d","bmisdirection3coco829007.7010005.501160031132121020010110700300280541715332255a2300a00239839f2238181818181818181818403325ff01ff06ff43ff223b33ff0144231924ff02ff3b33014442230c240404033b3321012501468aff032c3b4c4141414141414141414d","ccompact curse3ashe745009.5677006.867722103000d010004205626054a6101a01619229f3135535355373554245646456a7404a04742a381818181818181818403301216209242422ff3b3305fe8946464605013b3323232cff014346ff3b336442ff01ff4289423b33656462438a6362653b4c41414141414141414d"},{"dbring the heat3ashe416005.1677003.167722232130202300741e714381818181818403301696969013b33ff0a0a0a283b33010a0a0a013b33ff012101ff3b4c41414141414d","acold circle5willow519005.8344003.834421030020300300a5350a50531838181818181818403301ff696901053b33ff69696969013b330169696969ff3b3321016969ff053b4c4141414141414d","bfrozen water3mika624006.5011004.501122632530406300811e8a1260a60121c3818181818181818403301ff696969ff053b33ff48acb5a401ff3b33010591968dff013b33ff0169696901213b4c414141414141414d","cnot so simple4riley546005.5344003.85102112011050120025054811e8a2201a01221838181818181818403301ff01ff01223b3365488eb0a4013b33232165918dff3b332323ff01ff873b4c4141414141414d","ahot diamond3mika523006.8344004.83444300301002012030000025054724e71e381818181818184033210a7a8272223b33ff01578184723b3301ff017c86743b33ff0189575c543b3323ff28ff0a394d4c41414141414d4e","afreeze keys3ashe644006.5011004.5011216316206063009009f26154a1260a6012203541c381818181818181840332cff01ff01ff053b33ff0169692324223b33010569692324013b3322018a01ff01213b4c414141414141414d","alava bridge4riley647006.5677004.734431622611510606200810e8a6303a036322454244541d4e381818301818404e381048241a24ff134033ff01ff0cff01ff3b3301ff895089ff213b330501ff19ff01053b4c31ff221a22ff394d4e4c41414b41414d4e","ftiny contraption5autumn253004.1100002.344021231220502300701e7821e8722e7813e80d3818181840336969503b332850483b336989283b338748213b4c4141414d","9lava coins3coco625006.5011004.51775003113212102101060030041056730e74505626154433561c38181818181818184033016201280a620a3b33ff24ff017a72223b33016463558077523b332164236257540a3b4c414141414141414d","ahot springs3ashe810010.2677007.33442101000090010025054811e8761e7813e8a6320a2063a8404a04842d3818181818181818181840330166050606220606663b332148acb5b5a4285d693b3301ffaeb9b9a6235e693b33ff489196968d0551693b332555520a0aff01ff253b4c4141414141414141414d","ccool crossing3mika622007.9344005.68444002012122121090020042056831e8712e745256814e8a2562a622544656234e4e38181818404e4e4e4e3362ff013b4e4e381810ff48461318403321280a896962053b33ff4669896901ff3b3301488e928bff013b4c41310501ff39414d4e4e336363623b4e4e4e4e4c4141414d4e4e","8iced maze3ashe843012.6010009."},{"dfollow the gem3ashe841010.9677007.","dunlocking gems3ashe714012.8344009.284423003010c000005200755006352555720552300543035040120654526025460417653383818181818181818184033214aefff43cf44443b33ff4aff01430144443b33014a4a4a434201af3b33ff010f236f0403033b332fff0123230403033b33ff01ff232301ff013b3322ff4f238fff01023b4c41414141414141414d","boctogem maze3roxy613011.2010008.","fice fire islands3coco848016.5677011.","ckey labyrinth5autumn754011.2010008.","bweaving path5willow821017.3440012.","bfour islands3ashec15030.3344023.","bvictory laneacyansorceryf55040.6780032."}}function fv(f)if(ff==f)return false
holdframe()I(0,0,_[1][f],sget,sset)I(0,0,_[2][f],pget,pset)memcpy(12288,24576,255)ff=f end function fw(f)if(ft==f)return false
music(-1)holdframe()I(0,0,_[3][f],pget,pset)memcpy(12544,24576,128)memcpy(12800,24704,2176)ft=f end function fk(f,e,n)holdframe()I(e,n,_[4][f],mget,mset)end function fS(f)holdframe()I(0,0,_[5][f],pget,pset)memcpy(14976,24576,2176)end function el(f)holdframe()I(32,72,_[6][f],sget,sset)end function I(c,e,f,b,r)local function i(f,a)local e,n=f[1],1while(e~=a)n+=1e,f[n]=f[n],e
f[1]=a end local function d(e)local n=$f>>f%1*8<<32-e>>>16-e f+=e>>3return n end local function f(e)local f=0repeat f+=1local n=d(f)e+=n until n<(1<<f)-1return e end local t,u,s,n,l,a,o=f"0",f"0",f"1",{},{},0for f=1,f"1"do add(n,d(s))end for d=e,e+u do for c=c,c+t do a-=1if(a<1)a,o=f"1",not o
local a=d>e and b(c,d-1)or 0local e=l[a]or{unpack(n)}l[a]=e local f=e[o and 1or f"2"]i(e,f)i(n,f)r(c,d,f)end end end function er()local f={}f.obj_list,f.player,f.g_p_zap_turn,f.g_tile_count,f.playfield=fD(g),fD(g[1]),F,Q,et()add(G,f)if(count(G)>60)deli(G,1)
end function e7()local e=count(G)if(e==0)return
local f=G[e]if(e>0)deli(G)
g=f.obj_list g[1]=f.player F,Q=f.g_p_zap_turn,f.g_tile_count local e=g[1]e.nextdir,e.isdead,e.ismv,z,f1,M=-1,false,false,false,true,true eu(f.playfield)end function et()local f={}for e=0,34do f[e]={}for n=0,31do f[e][n]=mget(e,n)end end return f end function eu(n)for f=0,34do for e=0,31do mset(f,e,n[f][e])end end end function fD(e)local f={}for e,n in pairs(e)do f[e]=n end return f end function c(n,f,a,e)return tonum(sub(n,f,f+(e and e or 0)),a)end function L(n,f)local a,o,e={},#n\f for o=1,o do e=(o-1)*f a[o]=K(sub(n,e+1,e+f))end return a end function K(f)local e={}for n=1,#f do add(e,c(f,n,1))end return e end function p(f,e,n)return f+(e-f)*n end function v(a,o,d,e)for n=0,1do for f=0,1do if(e+f<32)mset(d+n,e+f,mget(a+n,o+f))
end end end function R(a,o,f,e)local e,n,d,c,i,l,f=i.s_w<<1,i.s_h<<1,48+f\16<<1,f%16<<1,48+e\16<<1,e%16<<1for e=1,e,2do for n=1,n,2do f=mget(e+1,n)if f==a then v(i,l,e,n)elseif f==o then v(d,c,e,n)end end end end function S(f,e)local n=mget(f,e)if(n~=125and n~=127)return 0
v(126,24,f-1,e)return 1+S(f-2,e)+S(f+2,e)+S(f,e-2)+S(f,e+2)end function fb(e,n,a,o,d)local f=#e for f=1,f do n=print(sub(e,f,f),n,a+sin(f2+f/10)*d,o)end end function y(f,e,n,a,o)?f,e+1,n+1,o
return print(f,e,n,a)end function fE(f)return(f>=60and flr(f/60)..":"or"")..sub(flr(f%60)+100,2,3).."."..sub(flr(f%1*1000)+1000,2,4)end function en()local f=j*a==1f8+=.075f8%=1fa+=.015fa%=1if(x==45and f5==false)music(h(5)and 0or-1,0,7)f5=true
if x<=45and f then if(btn(6))poke(24368,1)
if((Z and l or btn(6))and count(d)==0)es()
end O=mid(0,count(d)==0and(l or z)and O+.2or O-.2,1)if(f)for f in all(g)do f:onstep()end eh()
local f,e,n=g[1],i.s_w<<4,i.s_h<<4local a=f.inprt and cos(1-f.anim>>2)or f.anim X=i.s_w>=8and mid(0,(p(f.oldx,f.x,a)<<4)-48,e-112)or(e>>1)-56Y=i.s_h>7and mid(-8,(p(f.oldy,f.y,a)<<4)-48,n-108)or(n>>1)-60end function e5()poke(24405,0)if(fe)ex()else ey()
if(M)em()
if(f1)eg()
ep()poke(24405,96)palt(0)local f,e=-32+n,-n if(k>=3)f,e=-32+-X/2,-32+-Y/2
for f=f,128,32do for e=e,128,32do spr(148,f,e,4,2)spr(152,f,e+16,4,2)end end poke(24404,96)pal()if(k<=2)for f=0,127do sspr(0,f,128,1,sin(fa+(f>>4))*1.4,f)end
poke(24404,0)camera(X,Y)local f,e=i.s_w*2+2,i.s_h*2+2palt(256)map(0,0,0,0,f,e,1)fG()map(0,0,-7,0,f,e,128)fG()for f in all(fd[f0])do f:ondraw()end palt(16384)map(0,0,0,0,f,e,12)palt()map(0,0,0,0,f,e,2)for f in all(g)do f:ondraw()end ew()camera(0,0)local a,o=h(1),count(d)local f,e=p(127,115,O),a and 0or 87poke(24404,96)pal(fs)if o>0then sspr(0,0,128,128,0,0)else sspr(e,0,128,9,e,0)if(O>0)sspr(0,f,128,16,0,f)
end pal()poke(24404,0)local e=B<=i.s_gt and 7or 13if(l and B<=i.s_dt)e=10
local n=dget(i.s_st)spr(n<599.995and 147or 146,88,0)if a then?fE(B),2,2,e
if(fq)fb("new time!",32,2,e,1.3)
if(n<=i.s_gt)?"⧗",81,2,9
if(n<=i.s_dt)?"♥",75,2,14
end local e=g[1].ttch rect(96,1,126,7,7)fillp(f7[ceil(U)])rectfill(124-max(1,e/Q*26),3,124,5,59)fillp()if(l or z)fb(l and"stage clear!"or"❎ undo",l and 42or 50,f+5,7,1.3)
if o==1then local f=sin(d[1].m_anim_factor>>2)*-32+-24local e=y("stage "..C,f,106,7,0)line(f-2,112,f+e,112,7)y(i.s_nm,f,115,7,0)y(i.s_at,f,122,13,0)elseif not l and k==1and C==1and e>5then y("🅾️ sprint",12,12,7,0)end end function ex()local f="0c38010f00380438080f05380038010f0438113b0f01587f113c0f08113b587f0f0111432438010f00382538080f24380038010f2c3831430f01587f313b0f08313c587f0f01313b"for e=1,144,12do sspr(c(f,e,1,1),c(f,e+2,1,1),c(f,e+4,1,1),c(f,e+6,1,1),c(f,e+8,1,1),c(f,e+10,1,1))end line(88,127,104,127,0)line(0,56,0,71,0)end function ey()local o,e,f,n,a=fj,0,0,false,false for e=0,10do f,n,a=21+e,o&1<<e>0,o&8<<e>0sset(f,98,n and 7or 12)sset(f,96,a and 7or 0)sset(f,92,n and 9or 4)sset(f,90,o&32<<e>0and 10or 9)sset(f,88,a and 10or 0)end for e=0,4do f,n,a=8+e,o&16<<e>0,o&64<<e>0sset(f,98,n and 7or 12)sset(f,96,a and 7or 0)sset(f,90,n and 10or 9)sset(f,88,a and 10or 0)end for d=0,7do f,n,a=103-d,95-d,88+d e=o&1<<d>0and 7or 12sset(f,101,e)sset(50,103-d,e)sset(78,96+d,e)e=o&1<<d>0and 9or 4sset(f,91,e)sset(52,n,e)sset(76,a,e)e=o&4<<d>0and 9or 10sset(f,93,e)sset(50,n,e)sset(78,a,e)end fj>><=1end function ep()local a=f8 spr(224,80,104,6,2)local f,n=80for o=0,32,16do clip(f,104,16,16)for e=0,15do n=sin(a+e/8)+104sspr(o+e,112,1,16,f+e,n)sspr(f,104+e,16,1,f,n+e)end f+=16end local e=g[1]clip()palt(0,false)spr(208,24,104,3,1)palt()if(e.haskey)spr(144,32,104)spr(144,39,104,1,1,true)
for n=0,16,8do f=24+n clip(f,104,8,8)for e=0,7do sspr(e+n,104,1,8,f+sin(a+e/8)+e,104)end end clip()if(e.haskey)spr(145,32,104)spr(145,39,104,1,1,true)
local e,n,f=(btn(4)or e.sprint)and e.oncnv==false,e.blink<4or l f=e and 214or 215if(n or z)f=216
spr(f,l and 37or 36,104,1,1,l)f=e and 252or 251if(z)f=160
if(l)f=250
spr(f,40,104)for f=0,23do for e=0,7do sset(79-e,104+f,sget(24+f,104+e))end end end ev=L("1d5cdc125848145949",6)function em()local f,n for e=1,3do n,f=((F+3-e)%3<<1)+1,ev[e]pal(f[3],f[n])pal(f[4],f[n+1])spr(155+e,96+(e<<3),8,1,2)end pal()M=false end function fG()poke(24405,0)camera(0,0)local f,e for n=0,34do f,e=(n<<3)%128,(n\16<<4)+8sspr(f,e,8,16,f,e,8,16,true)end camera(X,Y)poke(24405,96)end function eg()local o,e,a,f=g[1].coins for n=0,1do e,a=4,5if(o>n)e,a=9,4
f=17-n*3for n=0,8,8do line(72+n,f,76+n,f,e)line(72+n,f-1,75+n,f-1,e)f+=1e=a end end f1=false end function eb(e)local f={type=c(e,1,1),x=c(e,2,1),y=c(e,3,1),spr=c(e,4,1,1),anim=rnd(),spin=rnd()}f.poskey=f.x<<4|f.y if f.type==0then f.stx,f.sty,f.poskey,f.dir,f.anim,f.jgl,f.sprint,f.pstate,f.ismv,f.oldx,f.oldy,f.strnx,f.strny,f.haskey,f.oncnv,f.inprt,f.blink,f.octg,f.coins,f.nextdir,f.ttch,f.isdead,f.prvsltr,f.prvcrfl=f.x,f.y,-1,3,1,0,false,0,false,f.x,f.y,f.x,f.y,false,false,false,10,0,0,-1,1,false,false,false f.onstep=ek f.ondraw=ez else if(f.type==5)f.oct_ind=f.spr f.spr=87
if(f.type==10)f.dst_x,f.dst_y=c(e,4,1),c(e,5,1)
function f:onstep()if(self.type<10)self.anim+=.02self.anim%=1self.spin+=.035self.spin%=1
end function f:ondraw()local f,e=(self.x<<4)+8,self.y<<4if self.type<9then if self.oct_ind==nil or self.oct_ind==g[1].octg then local e,a,o,n=e+sin(self.anim)*2,self.spr%16<<3,self.spr\16<<3,ceil(sin(self.spin*.5)*-8)if(n<=3)rectfill(f+7,e+1,f+9,e+14,7)
sspr(a,o,8,16,f+9-n,e,n,16)sspr(a,o,8,16,f+8,e,n,16,true)end elseif self.type<10then spr(self.spr,f+4,e+sin(self.anim)*2,1,2)end end end return f end function ek(f)f.blink-=1if(f.blink<=1)f.blink=30+flr(rnd(60))
if(not h(6)or count(d)>0)return
Z=btn(4)and fo==false fo=btn(4)f.jgl=max(f.jgl-.2,0)if(btnp(5)and not l)e7()return nil
f.anim=min(f.anim+((f.sprint or f.oncnv or f.inprt)and.2or.1111),1)if f.anim>=.65then local n=K"2013"for e=0,3do if(btn(e))f.nextdir=n[e+1]
end end if f.anim+.001>=1and not l and not z then if(f.ismv)eF(f)
if l==false and f.ttch>=Q then l,m=true,62483if(B<dget(i.s_st))dset(i.s_st,B)fq=true
end if(Z)f.jgl=1
f.sprint=false local e=f.nextdir if e~=-1or f.inprt then if(f.inprt)e=-1
local a,o,d,c=f.inprt,16,f.x+cos(e>>2),f.y+sin(e>>2)if(f.oncnv or h(2))o=18
local n=mget((d<<1)+2,(c<<1)+1)if(n==121and f.pstate==1or n==51and f.haskey or fget(n)&o>0)a=true
f.nextdir,f.dir=-1,e if a and not z and not l then fA=true if(m==nil and stat(49)==-1and not f.oncnv)m=61956
if not f.oncnv then if(not f.inprt)f.strnx,f.strny=f.x,f.y er()
F+=1F%=3M=true end f.anim,f.sprint,f.ismv=0,tonum(h(3))~tonum(btn(4))==1,true if(f.inprt==false)f.oldx,f.oldy=f.x,f.y
if(e>=0)f.x,f.y=d,c
end end end if(l)f.dir=3
if(fA and not l)B=min(B+(time()-fn),599.999)
end function eF(f)f.ismv=false local a,o,d,c,l,h=f.x,f.y,(f.oldx<<1)+2,(f.oldy<<1)+1,true,true if(f.prvcrfl)v(96,18,d-1,c)Q-=S(d,c)f.ttch-=1m=62278f.prvcrfl=false
local e,y,x,b,r,t,u,s,n,i=mget((a<<1)+2,(o<<1)+1),mget(d,c),a<<4|o,(a<<1)+1,(o<<1)+1,(a<<4)+12,o<<4if(e\16==12and f.pstate==2)e,m=123,62084
if(e==52)R(19,20,3,4)m,n=-349,"78e"
if(e==53)R(21,22,35,36)m,n=-349,"73b"
if(e==54)R(23,24,67,68)m,n=-349,"7dc"
if e==55then f.coins+=1m,n=63494,"49a"if(f.coins==3)f.coins,m=0,-349R(25,26,99,100)
f1=true end if(f.prvsltr)v(126,26,d-1,c)m=62084
f.prvsltr=e==48for n=0,2do if(e==80+n)i=f6[n+1]f.pstate,m=n,fx[n+1]
end for n=0,7do if e==56+n then if f.octg==n then f.octg+=1m=fP[n+1]for e in all(g)do if(e.oct_ind==f.octg)e_(t,u+6,(e.x<<4)+12,(e.y<<4)+6)
end else l=false end end end if(f.octg==8)R(27,28,74,106)f.octg=0
if(e==18)if f.haskey then l=false else f.haskey,n,m=true,"67",63494end
if(e==51)f.haskey,m=false,-349
if(e==125or e==127)f.prvcrfl,m=true,62210
if(e==121)m=fx[2]
local d=-1for f=0,3do if(e==113+(f<<1))d=f
end if(e==123)d=f.dir
if(d~=-1)f.nextdir=d
f.oncnv=d~=-1for e,f in pairs(g)do if f.poskey==x then s=f if(l)deli(g,e)
break end end if f.inprt then f.inprt,i=false,f6[f.pstate+1]else for n=0,6,2do if(e==89+n)v(126,f.pstate<<1,b,r)f.oldx,f.oldy,f.inprt,m,f.x,f.y=a,o,true,63142,s.dst_x,s.dst_y
end end if(e==123and f.pstate==1)e=192v(112,14,b,r)
if(e>208)fH(f)h=false
if e\16==11and f.pstate==0or e\16==12or e==50or e==29and F==0or e==30and F==1or e==31and F==2then fH(f,true)elseif h then f.ttch+=1v(126,f.pstate<<1,b,r)end if(n~=nil)fI(t,u+8,K(n))
if(i~=nil)fJ(t,u+12,i)
end function fH(f,e)z=true if(e)m=61448f.isdead=true fJ((f.x<<4)+12,(f.y<<4)+12,f6[f.pstate+1])
end function ez(f)if(f.isdead or f.inprt or not h(6))return
local d,e,n,a,o=f.dir,f.x<<4,f.y<<4,f.anim if(f.pstate==1)pal(3,8)pal(11,9)
if(f.pstate==2)pal(3,13)pal(11,6)
if(a<1)o=f.oncnv and a or.5+cos(a*.5*sgn(a-.5))*-.5e,n=p(f.oldx<<4,f.x<<4,o),p(f.oldy<<4,f.y<<4,o)
e+=(-1+rnd(3))*f.jgl+8n+=(-1+rnd(3))*f.jgl+8local a,o=cos(f.dir>>2)<0,sin(f.dir>>2)<0local d,c=a and e or e-8,o and n or n-8if(f.dir%2==0)spr(211,d,n+7,3,1,a)spr(211,d,n,3,1,a,true)else spr(217,e+1,c,1,3,false,o)spr(217,e+8,c,1,3,true,o)
pal()end function fR()cartdata("cs_blobun_"..e9)if(dget(63)==0)dset(63,117)for f=0,59do dset(f,599.999)end dset(62,0x1.12)
end function h(f)return dget(63)&1<<f>0end function ej(f,n)local f,e=1<<f,dget(63)dset(63,n and e|f or e&~f)end function fz()return{stage=dget(62)<<4&15,world=dget(62)<<7&7}end function e6(f,e)dset(62,dget(62)&~.9922|f>>7|e>>4)end function f3(e)local f=0for e in all(r[e])do if(dget(e.s_st)<599.995)f+=1
end return f end function fi(f)return f3(f)>=J[f]end function fK(f)return f3(f)>=count(r[f])end function fB()local f=count(r)return f3(f)>=J[f]end function f9(f,e)for f in all(r[f])do if(dget(f.s_st)>(e and f.s_dt or f.s_gt))return false
end return true end function fc()for f=1,count(r)do if(not fK(f))return false
end return true end function H(f)for e=1,count(r)do if(not f9(e,f))return false
end return true end function ef()cls(0)map(0,0,-24*s,32,19,2)map(0,2,s*2%1*-24,48,19,3)map(0,5,s*3%1*-24,72,19,7)palt(1024)local f=sin(s)*4map(19,0,12,8+f,13,7)if(not h(6))map(19,8,28,8+f,4,5)
?"2025 cyansorcery",32,122,3
?"v1",1,1,1
palt()pal(7,1)if(fc())spr(241,1,119)
if h(1)then if(H(false))?"⧗",113,122,7
if(H(true))?"♥",120,122,7
end pal()if(count(d)==0and q==nil)fb("press 🅾️ to start!",28,96,1,1.4)
end function e3()local e=j*a local f=e*8cls(1)rectfill(0,56,127,72,12)map(0,12,-24*s,56-f,19,1)map(0,13,-24+24*s,64+f,19,1)local f=f*.5?"presented by",39,60-f,1
if(h(6))map(19,7,18,60+f,11,1)
palt()fillp(fu[flr(p(1,4,e))])rectfill(0,56,127,72,12)fillp()end function fF()if(j*a==1and btnp()&48>0)if D==5then u(1)else u(0)end
end function f_()for f=-32+n,128,32do for e=-n,128,32do spr(204,f,e,4,4)end end poke(24404,96)pal(fs)if(D==nil)sspr(0,58,128,70,0,58)sspr(0,7,128,18,0,7)sspr(39,38,52,20,39,38)else sspr(0,0,128,128,0,0)
pal()poke(24404,0)palt(4096)if(D==nil)map(48,0,0,0,16,16)local n={{"programming","bug fixes","general"},{"art, music","art, music","music"}}for f=1,2do for e=1,3do y(n[f][e],f*64-46,e*24+42,7,1)end end y("director",58,42,7,1)y("created by",44,1,7,1)y("a plural system",34,26,7,1)y("seven spirits, one body",18,32,7,1)else map(32,8+D*2,0,32,16,2)local f={[[ you've overcome many
 challenges and are at
the end of your journey!

but many puzzles remain
unsolved. can you solve
      them all?]],[[you're very good! you've
overcome every challenge
 in your way, and have
 solved all the puzzles.

       good job!]],[[ wow, you're fast! not
only did you solve every
puzzle, but you've done
 it super quick as well.

    congratulations!]],[[ we have no more puzzles
for you. you're too good
  at the game! not many
   people can do this.

    you are amazing!]],[[  your exploration has
  led to the discovery
     of a new area!
       good luck!]]}y(f[D],16,56,7,0)
end function e0()s+=.025s%=1if W>0then W-=1if(btnp()&48>0)W=0
if(W==0)u(0)
end end function e4()s+=.01s%=1if count(d)==0then local f=btn(6)if(f)poke(24368,1)
if(btnp()&48>0or f)m=61828eq()
end end function e2()A=mid(0,A+E,1)s+=.02s%=1local f=count(r)if A==1then if(btnp(0)and b>1)m=61700N=max(b-1,1)E=-.125
if(btnp(1)and b<count(r)and fi(b))m=61700N=min(b+1,f)E=-.125
end if(A==0)E=.125b=N N=nil
local f,e=count(r[b]),btn(6)if(e)poke(24368,1)
if(btnp(2))w-=1m=61700
if(btnp(3))w+=1m=61700
if(w<0)w=f
if(w>f)w=0
if btnp()&48>0or e then if(w==0)u(0)m=63333else u(2,b,w)music(-1,500)m=61828
end end function e8()local a,f=ee,20+sin(A>>3)*29for f=1,4do pal(a[1][f],a[b][f])end rectfill(0,0,127,127,1)map(0,14,-24+24*s,-2-f,19,3)map(0,17,-24*s,115+f,19,2)map(32,-2+b*2,0,1-f,16,2)local f,e=122+f,J[b]if(b>1)?"⬅️previous area",1,f,7
local n=f3(b)?n>=e and(b<count(r)and"next area➡️"or"")or"▶solve "..e-n,84,f,7
local e=count(r[b])local o=max(min(w-5,e-9),-1)local n,d,i,c,l,f=13-o*10,o+10,h(1)for e=0,e do c,fL=e==w,n+10if e>o and e<d then f=n+3local o=r[b][max(1,e)]local d=dget(o.s_st)if(c)fM(4,n,90,fL)
pal(7,c and 7or a[b][2])if e>0then?(e>9and""or" ")..e.." "..o.s_nm,14,f,7
spr(d<=599.995and 241or 240,6,n+1)if i then if(d<=o.s_gt)?fE(d),96,f,7
if(d<=o.s_dt)?"♥",120,f,7
end else?"back",26,f,7
pal(7,10)if(fK(b))spr(241,6,n+1)
if i then if(f9(b,false))?"⧗",113,f,7
if(f9(b,true))?"♥",120,f,7
end end end n=fL end if A<1then palt()if(E<0)pal(1,a[N][1])
fillp(fu[flr(p(1,4,A))])if(E>0)rectfill(0,22,127,112,1)else rectfill(0,0,127,127,1)
fillp()end pal()end function fr(e,n,a,f)add(d,{m_x=e,m_y=n,m_w=a,m_h=count(f)*10+6,m_items=f,m_highlight=1,m_anim_incr=.25,m_anim_factor=0,m_step=function(f,n)local e=count(d)local n,a,o=n==e,btn(6),count(f.m_items)if(a)poke(24368,1)
if f.m_anim_factor==1then if(btnp(2))f.m_highlight-=1m=61700
if(btnp(3))f.m_highlight+=1m=61700
if(f.m_highlight<1)f.m_highlight=o
if(f.m_highlight>o)f.m_highlight=1
if(btnp(5)or btnp(4)or a)m=61828f.m_items[f.m_highlight]:i_onclick()
end local a=true if(n and e>1and d[e-1].m_anim_factor>0)a=false
if(a)f.m_anim_factor=mid(0,f.m_anim_factor+(n and f.m_anim_incr or-.25),1)
if(f.m_anim_factor==0and f.m_anim_incr<0)deli(d)
end,m_draw=function(f,n)palt()local e=count(d)local n=n==e if(n and e>1and d[e-1].m_anim_factor>0)return false
local e,t,a=count(f.m_items),count(g)>0and 147or 241,p(5,f.m_h,f.m_anim_factor)local n,e,o=f.m_x,f.m_y-(a>>1)local a,d,i,l,b,r=n+f.m_w,e+a rectfill(n,e,a,d,1)rect(n+1,e+1,a-1,d-1,13)local u,e=n+8,e+6for s,c in pairs(f.m_items)do if e<d-9then i,l,b,r=n+3,e-3,a-3,e+7o=s==f.m_highlight if(o)fM(i,l,b,r)
pal(7,o and 7or 13)?c.i_label,u,e,7
if(c.i_setting~=nil)spr(t-(h(c.i_setting)and 0or 1),a-11,e-2)
end e+=10end pal()end})end function fg()for f in all(d)do f.m_anim_incr=-.25end end function fN()m=63333d[count(d)].m_anim_incr=-.25end function o(f,e)return{i_label=f,i_onclick=e,i_setting=nil}end function f(f,e)local f=o(f,function(f)ej(f.i_setting,h(f.i_setting)==false)if(f.i_setting==5)music(h(5)and 0or-1,0,7)
end)f.i_setting=e return f end function es()Z,fo,m=false,false,61828local f if(not ei and fi(k))f=5
if(not ea and fB())f=1
if(not eo and fc())f=2
if(not ed and H(false))f=3
if(not ec and H(true))f=4
if(f~=nil)u(5,f)return
local e,n,a,f=o((l and"next"or"skip").." puzzle",function()if(C+1>count(r[k]))u(1)else u(2,k,C+1)
end),o("restart puzzle",function()u(2,k,C,true)end),o("stage select",function()u(1)end)if(l)f={e,n,a}else f={o("back",fN),n,e,o((f0==2and"hide"or"show").." hints",function()f0=f0==2and 1or 2fg()end),a,o("options",fO),o("go to title",function()u(0)end)}
fr(16,64,96,f)end function eq()fr(24,96,80,{o("start game",function()u(1)end),o("continue",function()local f=fz()u(2,f.world,f.stage)music(-1,500)end),o("options",fO),o("credits",function()u(4)end),o("back",function()u(3)end)})end function fO()fr(16,count(g)>0and 64or 72,96,{o("back",fN),f("show timers",1),f("slime overlap",2),f("sprint by default",3),f("sound effects",4),f("music",5),f("lesbians allowed",6),o("exit game",function()extcmd"shutdown"end),o("pico8 menu",eA)})end function eA()extcmd"pause"end function fM(f,e,n,a)rectfill(f,e,n,a,2)fillp(f7[ceil(U)])rect(f,e,n,a,154)fillp(0)end function f4(e,n,a,o,d,f)if(count(P)<100)add(P,{x=e,y=n,spd=a,dir=o,col=d,time_max=f,time=f})
end function fJ(d,c,o)local f,e,n for a=0,.9,.1do f,e,n=d+cos(a)*4,c+sin(a)*4,a*360f4(f,e,.9,n,rnd(o),12)f4(f,e,1.8,n,rnd(o),8)end end function e_(a,o,e,n)for f=0,.8,.2do f4(p(a,e,f),p(o,n,f),0,0,14,p(5,12,f))end fI(e,n,{14,7})end function fI(e,n,a)for f=0,.9,.1do f4(e+cos(f)*4,n+sin(f)*4,.8,f*360,rnd(a),8)end end function eh()for f in all(P)do local e=f.dir/360f.x+=f.spd*cos(e)f.y+=f.spd*sin(e)f.time-=1if(f.time<=0)del(P,f)
end end function ew()for f in all(P)do pal(7,f.col)spr(flr(p(163,161,f.time/f.time_max)+.5),f.x,f.y)end end
__gfx__

__map__

__gff__

__sfx__

__music__
0a 103b1976
08 090f4904
05 0324424b
0c 0c5c305b
0e 2f166324
0e 762f547e
06 70586330
0a 70400e65
02 07460f32
02 62762837
02 757f217f
06 5c077963
03 4e630f47
0d 6708317d
0c 30395230
0f 3a4d6a61
07 07391024
09 11265620
08 04132402
01 79245648
0a 125e3911
01 08052e0b
05 021f1a1d
0f 0f670867
05 70137967
0c 7e587703
03 2c094802
02 49134872
0b 6a767005
02 6c114407
0e 391a2719
04 7c7e1827
02 72697125
08 0e4d4272
0a 244c2410
0c 075f624b
0c 21197c61
0d 4e1f6c3b
07 746a1a7f
04 762d7901
03 007c2241
06 2c1e451f
05 191f484e
0d 6346271f
0c 38630b05
0d 7a2e160b
0a 6c463f78
01 422b330e
09 191d572f
07 634a617d
08 1e4b2e40
04 6c067047
09 281f2609
03 4378030c
0e 302e6862
05 1f507f5c
01 321c2435
00 06061837
01 606f7d2e
07 644e111f
06 4a56776d
06 736a7243
0a 376f0f0f
0e 3e316862
__label__

__meta:title__
blobun mini
by cyansorcery
