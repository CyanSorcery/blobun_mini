...
flip()memcpy(32768,0,17152)_={{},{},{},{},{},{}}ff,fr=0,0local e,f=32768,%32768while(f~=0)add(_[f>>13&7],e+2)e+=2+(f&8191)f=%e
...
local a=d>e and b(c,d-1)or 0local e=l[a]or{unpack(n)}l[a]=e local f=e[o and 1or f"2"]i(e,f)i(n,f)t(c,d,f)end end end function et()local f={}f.obj_list,f.player,f.g_p_zap_turn,f.g_tile_count,f.playfield=fD(g),fD(g[1]),F,Q,er()add(G,f)if(count(G)>60)deli(G,1)
end function e7()local e=count(G)if(e==0)return
local f=G[e]if(e>0)deli(G)
g=f.obj_list g[1]=f.player F,Q=f.g_p_zap_turn,f.g_tile_count local e=g[1]e.nextdir,e.isdead,e.ismv,v,f1,M=-1,false,false,false,true,true eu(f.playfield)end function er()local f={}for e=0,34do f[e]={}for n=0,31do f[e][n]=mget(e,n)end end return f end function eu(n)for f=0,34do for e=0,31do mset(f,e,n[f][e])end end end function fD(e)local f={}for e,n in pairs(e)do f[e]=n end return f end function c(n,f,a,e)return tonum(sub(n,f,f+(e and e or 0)),a)end function L(n,f)local a,o,e={},#n\f for o=1,o do e=(o-1)*f a[o]=K(sub(n,e+1,e+f))end return a end function K(f)local e={}for n=1,#f do add(e,c(f,n,1))end return e end function p(f,e,n)return f+(e-f)*n end function k(a,o,d,e)for n=0,1do for f=0,1do if(e+f<32)mset(d+n,e+f,mget(a+n,o+f))
end end end function R(a,o,f,e)local e,n,d,c,i,l,f=l.s_w<<1,l.s_h<<1,48+f\16<<1,f%16<<1,48+e\16<<1,e%16<<1for e=1,e,2do for n=1,n,2do f=mget(e+1,n)if f==a then k(i,l,e,n)elseif f==o then k(d,c,e,n)end end end end function S(f,e)local n=mget(f,e)if(n~=125and n~=127)return 0
...
local e,n,f=(btn(4)or e.sprint)and e.oncnv==false,e.blink<4or u f=e and 214or 215if(n or v)f=216
...
sspr(a,o,8,16,f+9-n,e,n,16)sspr(a,o,8,16,f+8,e,n,16,true)end elseif self.type<10then spr(self.spr,f+4,e+sin(self.anim)*2,1,2)end end end return f end function ek(f)f.blink-=1if(f.blink<=1)f.blink=30+flr(rnd(60))
if(not h(6)or count(d)>0)return
Z=btn(4)and fo==false fo=btn(4)f.jgl=max(f.jgl-.2,0)if(btnp(5)and not u)e7()return nil
...
end end local i=f.nextdir if f.anim+.001>=1and not u and not v then if(f.ismv)eF(f)
if not u and f.ttch>=Q and not v then u,m=true,62483if(B<dget(l.s_st))dset(l.s_st,min(599,B))fA=true
end if(Z)f.jgl=1
f.sprint=false local e=f.nextdir if e~=-1or f.inprt then::f::if(f.inprt)e=-1
local n,o,d,c=f.inprt,16,f.x+cos(e>>2),f.y+sin(e>>2)if(f.oncnv or h(2))o=18
local a=mget((d<<1)+2,(c<<1)+1)if(a==121and f.pstate==1or a==51and f.haskey or fget(a)&o>0)n=true
if(f.oncnv and not n)e,f.oncnv=i,false goto f
f.nextdir,f.dir=-1,e if n and not v and not u then fq=true if(m==nil and stat(49)==-1and not f.oncnv)m=61956
if not f.oncnv then if(not f.inprt)f.strnx,f.strny=f.x,f.y et()
F+=1F%=3M=true end f.anim,f.sprint,f.ismv=0,tonum(h(3))~tonum(btn(4))==1,true if(f.inprt==false)f.oldx,f.oldy=f.x,f.y
if(e>=0)f.x,f.y=d,c
end end end if(u)f.dir=3
if(fq and not u)B=min(B+(time()-fn),599.999)
...
local e,y,x,b,t,r,u,s,n,i=mget((a<<1)+2,(o<<1)+1),mget(d,c),a<<4|o,(a<<1)+1,(o<<1)+1,(a<<4)+12,o<<4if(e\16==12and f.pstate==2)e,m=123,62084
...
end for n=0,7do if e==56+n then if f.octg==n then f.octg+=1m=fP[n+1]for e in all(g)do if(e.oct_ind==f.octg)e_(r,u+6,(e.x<<4)+12,(e.y<<4)+6)
...
break end end if f.inprt then f.inprt,i=false,f6[f.pstate+1]else for n=0,6,2do if(e==89+n)k(126,f.pstate<<1,b,t)f.oldx,f.oldy,f.inprt,m,f.x,f.y=a,o,true,63142,s.dst_x,s.dst_y
end end if(e==123and f.pstate==1)e=192k(112,14,b,t)
if(e>208)fH(f)h=false
if e\16==11and f.pstate==0or e\16==12or e==50or e==29and F==0or e==30and F==1or e==31and F==2then fH(f,true)elseif h then f.ttch+=1k(126,f.pstate<<1,b,t)end if(n~=nil)fI(r,u+8,K(n))
if(i~=nil)fJ(r,u+12,i)
end function fH(f,e)v=true if(e)m=61448f.isdead=true fJ((f.x<<4)+12,(f.y<<4)+12,f6[f.pstate+1])
...
if(H(true))?"♥",120,122,7
end pal()if(count(d)==0and A==nil)fb("press 🅾️ to start!",28,96,1,1.4)
...
palt()fillp(fu[flr(p(1,4,e))])rectfill(0,56,127,72,12)fillp()end function fF()if(j*a==1and btnp()&48>0)if D~=nil then i(1)else i(0)end
...
end end function e2()q=mid(0,q+E,1)s+=.02s%=1local f=count(r)if q==1then if(btnp(0)and b>1)m=61700N=max(b-1,1)E=-.125
...
if(w<0)w=f
if(w>f)w=0
if btnp()&48>0or e then if(w==0)i(0)m=63333else i(2,b,w)music(-1,500)m=61828
end end function e8()local a,f=ee,20+sin(q>>3)*29for f=1,4do pal(a[1][f],a[b][f])end rectfill(0,0,127,127,1)map(0,14,-24+24*s,-2-f,19,3)map(0,17,-24*s,115+f,19,2)map(32,-2+b*2,0,1-f,16,2)local f,e=122+f,J[b]if(b>1)?"⬅️previous area",1,f,7
...
fillp(fu[flr(p(1,4,q))])if(E>0)rectfill(0,22,127,112,1)else rectfill(0,0,127,127,1)
fillp()end pal()end function ft(e,n,a,f)add(d,{m_x=e,m_y=n,m_w=a,m_h=count(f)*10+6,m_items=f,m_highlight=1,m_anim_incr=.25,m_anim_factor=0,m_step=function(f,n)local e=count(d)local n,a,o=n==e,btn(6),count(f.m_items)if(a)poke(24368,1)
...
local e,r,a=count(f.m_items),count(g)>0and 147or 241,p(5,f.m_h,f.m_anim_factor)local n,e,o=f.m_x,f.m_y-(a>>1)local a,d,i,l,b,t=n+f.m_w,e+a rectfill(n,e,a,d,1)rect(n+1,e+1,a-1,d-1,13)local u,e=n+8,e+6for s,c in pairs(f.m_items)do if e<d-9then i,l,b,t=n+3,e-3,a-3,e+7o=s==f.m_highlight if(o)fM(i,l,b,t)
pal(7,o and 7or 13)?c.i_label,u,e,7
if(c.i_setting~=nil)spr(r-(h(c.i_setting)and 0or 1),a-11,e-2)
...
end)f.i_setting=e return f end function es()Z,fo,m=false,false,61828local f if(not ei and fi(z))f=5
if(not ea and fB())f=1
if(not eo and fc())f=2
if(not ed and H(false))f=3
if(not ec and H(true))f=4
if(f~=nil)i(5,f)return
local e,n,a,f=o((u and"next"or"skip").." puzzle",function()if(C+1>count(r[z]))i(1)else i(2,z,C+1)
end),o("restart puzzle",function()i(2,z,C,true)end),o("stage select",function()i(1)end)if(u)f={e,n,a}else f={o("back",fN),n,e,o((f0==2and"hide"or"show").." hints",function()f0=f0==2and 1or 2fg()end),a,o("options",fO),o("go to title",function()i(0)end)}
ft(16,64,96,f)end function eA()ft(24,96,80,{o("start game",function()i(1)end),o("continue",function()local f=fz()i(2,f.world,f.stage)music(-1,500)end),o("options",fO),o("credits",function()i(4)end),o("back",function()i(3)end)})end function fO()ft(16,count(g)>0and 64or 72,96,{o("back",fN),f("show timers",1),f("slime overlap",2),f("sprint by default",3),f("sound effects",4),f("music",5),f("lesbians allowed",6),o("exit game",function()extcmd"shutdown"end),o("pico8 menu",eq)})end function eq()extcmd"pause"end function fM(f,e,n,a)rectfill(f,e,n,a,2)fillp(f7[ceil(U)])rect(f,e,n,a,154)fillp(0)end function f4(e,n,a,o,d,f)if(count(P)<100)add(P,{x=e,y=n,spd=a,dir=o,col=d,time_max=f,time=f})
