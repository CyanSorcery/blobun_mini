...
end end end function R(a,o,f,e)local e,n,d,c,i,l,f=l.r<<1,l.F<<1,48+f\16<<1,f%16<<1,48+e\16<<1,e%16<<1for e=1,e,2do for n=1,n,2do f=mget(e+1,n)if f==a then k(i,l,e,n)elseif f==o then k(d,c,e,n)end end end end function S(f,e)local n=mget(f,e)if(n~=125and n~=127)return 0
...
local f,e,n=g[1],l.r<<4,l.F<<4local a=f.d and cos(1-f.a>>2)or f.a X=l.r>=8and mid(0,(p(f.g,f.f,a)<<4)-48,e-112)or(e>>1)-56Y=l.F>7and mid(-8,(p(f.p,f.e,a)<<4)-48,n-108)or(n>>1)-60end function e5()poke(24405,0)if(fe)ex()else ey()
if(M)em()
if(f1)eg()
...
Z=btn(4)and fo==false fo=btn(4)f.w=max(f.w-.2,0)if(btnp(5)and not u)e7()return nil
f.a=min(f.a+((f.A or f.i or f.d)and.2or.1111),1)if f.a>=.65then local n=K"2013"for e=0,3do if(btn(e))f.h=n[e+1]
end end local i=f.h if f.a+.001>=1and not u and not v then if(f._)eF(f)
if not u and f.j>=Q and not v then u,m=true,62483if(B<dget(l.s))dset(l.s,min(599,B))fA=true
end if(Z)f.w=1
f.A=false local e=f.h if e~=-1or f.d then::f::if(f.d)e=-1
local n,o,d,c=f.d,16,f.f+cos(e>>2),f.e+sin(e>>2)if(f.i or h(2))o=18
local a=mget((d<<1)+2,(c<<1)+1)if(a==121and f.c==1or a==51and f.x or fget(a)&o>0)n=true
if(f.i and not n)e,f.i=i,false goto f
f.h,f.o=-1,e if n and not v and not u then fq=true if(m==nil and stat(49)==-1and not f.i)m=61956
if not f.i then if(not f.d)f.ff,f.f2=f.f,f.e et()
...
f.S=e==48for n=0,2do if(e==80+n)i=f6[n+1]f.c,m=n,fx[n+1]
...
break end end if f.d then f.d,i=false,f6[f.c+1]else for n=0,6,2do if(e==89+n)k(126,f.c<<1,b,t)f.g,f.p,f.d,m,f.f,f.e=a,o,true,63142,s.f8,s.fe
...
end function fH(f,e)v=true if(e)m=61448f.G=true fJ((f.f<<4)+12,(f.e<<4)+12,f6[f.c+1])
...
end return f end function fi(f)return f3(f)>=J[f]end function fK(f)return f3(f)>=count(r[f])end function fB()local f=count(r)return f3(f)>=J[f]end function f9(f,e)for f in all(r[f])do if(dget(f.s)>(e and f.z or f.k))return false
...
fillp()end pal()end function ft(e,n,a,f)add(d,{fn=e,f5=n,f6=a,fa=count(f)*10+6,C=f,b=1,D=.25,l=0,U=function(f,n)local e=count(d)local n,a,o=n==e,btn(6),count(f.C)if(a)poke(24368,1)
...
end e+=10end pal()end})end function fg()for f in all(d)do f.D=-.25end end function fN()m=63333d[count(d)].D=-.25end function o(f,e)return{fd=f,fo=e,m=nil}end function f(f,e)local f=o(f,function(f)ej(f.m,h(f.m)==false)if(f.m==5)music(h(5)and 0or-1,0,7)
end)f.m=e return f end function es()Z,fo,m=false,false,61828local f if(not ei and fi(z))f=5
