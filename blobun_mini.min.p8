...
f9,f7={51510,27795,14025,37740},{32769.5,42405.5,60855.5,-.5}f2,fe,n,T,U,f8,fn,_,o,d,fo,y=0,0,0,0,0,true,time(),0,1,{},false fu,fs,fa,I,fO,fh=J"0001115525511125",K("3c26915154",5),K("13b49a5d6",3),J(I),{64515,-925,-829,-733,-637,-541,-445,-248},{64140,63686,63880}fP()fQ()fR(1)fx()end function _update()f8=f8==false U+=.2U%=4f2+=.035f2%=1n+=.15n%=32T-=1if T<=0then T=14elseif T%7==1then L=true end fS()x=max(x-1)if(btnp()&48>0and x>45)x=45
...
return print(f,e,n,o)end function fD(f)return(f>=60and flr(f/60)..":"or"")..sub(flr(f%60)+100,2,3).."."..sub(flr(f%1*1000)+1000,2,4)end function e8()local f=_*o==1fe+=.075fe%=1fd+=.015fd%=1if(x==45and fo==false)music(h(5)and 0or-1,0,7)fo=true
...
local f,e,n=g[1],i.s_w<<4,i.s_h<<4local o=f.inprt and cos(1-f.anim>>2)or f.anim X=i.s_w>=8and mid(0,(p(f.oldx,f.x,o)<<4)-48,e-112)or(e>>1)-56Y=i.s_h>7and mid(-8,(p(f.oldy,f.y,o)<<4)-48,n-108)or(n>>1)-60end function en()poke(24405,0)if(f8)es()else eh()
if(L)ex()
if(f1)em()
...
if(n<=i.s_gt)?"⧗",81,2,9
if(n<=i.s_dt)?"♥",75,2,14
end local e=g[1].ttch rect(96,1,126,7,7)fillp(f9[ceil(U)])rectfill(124-max(1,e/Q*26),3,124,5,59)fillp()if(l or k)fl(l and"stage clear!"or"❎ undo",l and 42or 50,f+5,7,1.3)
...
palt()fillp(f7[flr(p(1,4,e))])rectfill(0,56,127,72,12)fillp()end function fz()if(_*o==1and btnp()&48>0)s(0)
...
end)f.i_setting=e return f end function e7()Z,f6,y=false,false,61828local f if(not ea and fA())f=1
