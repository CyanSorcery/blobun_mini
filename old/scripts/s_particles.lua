...
function part_create(_x, _y, _spd, _dir, _col, _time)
 add(g_p_list, {x=_x, y=_y, spd = _spd, dir=_dir, col=_col,time_max=_time,time=_time})
end
...
function part_create_slime_explode(_x, _y, _col)
 ...
 for i=0,.9,.1 do
  ...
  part_create(_xfin, _yfin, .9, _deg, rnd(_col), 12)
  part_create(_xfin, _yfin, 1.8, _deg, rnd(_col), 8)
 end
end
...
function part_create_octogem(_x1, _y1, _x2, _y2)
 local _xfin, _yfin
 for i=0,.8,.2 do
  part_create(lerp(_x1, _x2, i), lerp(_y1, _y2, i), 0, 0, 14, lerp(5, 12, i))
 end
 part_create_item_grab(_x2, _y2, {14, 7})
end
function part_create_item_grab(_x, _y, _col)
 for i=0,.9,.1 do
  part_create(_x + (cos(i) * 4), _y + (sin(i) * 4), .8, i * 360, rnd(_col), 8)
 end
end
