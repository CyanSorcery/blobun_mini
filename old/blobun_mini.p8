...
function _init()
 ...
 -- figure out the data, offset, and length, store it, then move to the next block
 while _data != 0 do add(g_px9_data[_data >> 13&0x7], _offset+2) _offset += 2 + (_data&0x1fff) _data = %_offset end
...
function _draw()
 ...
 if g_s_intro_cd > 0 then
  ...
  print_shd(g_p_fst.l_name, _o1, 107, 7, _colshd)
...
function unpack_level(_world, _stage)
 ...
 for i=1,_data_len do
  _offset += 2
  _dstile = subl(_data, _offset, 1, 0x1)
...
function place_puzz_tile(_x, _y, _tile_id)
 ...
 local _tile_tl = subl(g_tile_lut, _offset, 1, 0x1)
...
function parse_levels()
 ...
 for _w, _wt in ipairs(g_levels) do
  ...
  for _s, _str in ipairs(_wt) do
   ...
   for i=1,2 do
    _bytes = subl(_str, _offset, 0, 0x1)
    _fst[_strs[i]] = sub(_str, _offset + 1, _offset + _bytes)
    _offset += _bytes + 1
   end
   -- get the stage save slot
   _fst.l_saveslot = subl(_str, _offset, 1)
   -- get the stage goal time and dev time
   _fst.l_goaltime = subl(_str, _offset + 2, 7)
   _fst.l_devtime = subl(_str, _offset + 10, 7)
   _offset += 18
   -- get the stage width
   _fst.l_width = subl(_str, _offset, 0, 0x1)
   _offset += 1
   -- get the stage hints
   _hints = subl(_str, _offset, 0, 0x1)
   _bytes = ceil(_hints / 2)
   _fst.l_hintcount, _fst.l_hintstr = _hints, _bytes == 0 and "" or sub(_str, _offset, _offset + _bytes)
...
function unpack_hints()
 ...
 for i=0,_hintcount do
  _offset = 1 + i\2
  _dir = subl(_hintstr, _offset, 0, 0x1) >> _bo & 0x3
