...
self.nextdir,self.dir=-1,_new_dir if _can_move and not g_stage_lose then g_p_started=true if(g_play_sfx==nil and stat(49)==-1and not self.onconvey)g_play_sfx=61956
...
end end end if g_stage_win==false and self.tilestouched>=g_tile_count then g_stage_win,g_play_sfx=true,62483if(g_p_time<dget(g_p_sst.s_saveslot))dset(g_p_sst.s_saveslot,g_p_time)g_p_new_time=true
...
if _tile==55then self.coins+=1g_play_sfx,_partcol=63494,{4,9,10}if(self.coins==3)self.coins,g_play_sfx=0,-349tile_swap(25,26,99,100)
g_p_updt_coin=true end if(self.prevslimetrap)tile_copy(126,26,_oldx-1,_oldy)g_play_sfx=62084
self.prevslimetrap=_tile==48for i=0,2do if(_tile==80+i)_ppartcol=g_pal_state_part[i+1]self.pstate,g_play_sfx=i,g_sfx_pstate[i+1]
end for i=0,7do if _tile==56+i then if self.octogems==i then self.octogems+=1g_play_sfx=g_sfx_octo[i+1]for _o in all(g_list_obj)do if(_o.oct_ind==self.octogems)part_create_octogem(_visx,_visy+6,(_o.x<<4)+12,(_o.y<<4)+6)
end else _destroy_obj=false end end end if(self.octogems==8)tile_swap(27,28,74,106)self.octogems=0
if(_tile==18)if self.haskey then _destroy_obj=false else self.haskey,_partcol,g_play_sfx=true,{6,7},63494end
if(_tile==51)self.haskey,g_play_sfx=false,-349
if(_tile==125or _tile==127)self.prevcrackedfloor,g_play_sfx=true,62210
...
break end end if self.inportal then self.inportal,_ppartcol=false,g_pal_state_part[self.pstate+1]else for i=0,6,2do if(_tile==89+i)tile_copy(126,self.pstate<<1,_tcp_dx,_tcp_dy)self.oldx,self.oldy,self.inportal,g_play_sfx,self.x,self.y=_x,_y,true,63142,_collision_obj.dst_x,_collision_obj.dst_y
