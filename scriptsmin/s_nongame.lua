...
palt()fillp(g_fillp_transition[flr(lerp(1,4,_baseanim))])rectfill(0,56,127,72,12)fillp()end function update_credits()if(g_intro_anim*g_outro_anim==1and btnp()&48>0)if g_victory_mode~=nil then set_game_mode(1)else set_game_mode(0)end
end function draw_credits()for _x=-32+g_bg_scl,128,32do for _y=-g_bg_scl,128,32do spr(204,_x,_y,4,4)end end poke(24404,96)pal(g_pal_dark)if(g_victory_mode==nil)sspr(0,58,128,70,0,58)sspr(0,7,128,18,0,7)sspr(39,38,52,20,39,38)else sspr(0,0,128,128,0,0)
...
for you. you're too good
  ...
  led to the discovery
     of a new area!
       good luck!]]}print_shd(_str[g_victory_mode],16,56,7,0)
...
if(btnp()&48>0or _pb)g_play_sfx=61828menu_create_title()
end end function update_stage_select()g_sss_anim_factor=mid(0,g_sss_anim_factor+g_sss_anim_incr,1)g_title_scroll+=.02g_title_scroll%=1local _worlds=count(g_levels)if g_sss_anim_factor==1then if(btnp(0)and g_sss_menu_world>1)g_play_sfx=61700g_sss_menu_world_tgt=max(g_sss_menu_world-1,1)g_sss_anim_incr=-.125
if(btnp(1)and g_sss_menu_world<count(g_levels)and stages_beat_min(g_sss_menu_world))g_play_sfx=61700g_sss_menu_world_tgt=min(g_sss_menu_world+1,_worlds)g_sss_anim_incr=-.125
...
if(btnp(2))g_sss_menu_stage-=1g_play_sfx=61700
if(btnp(3))g_sss_menu_stage+=1g_play_sfx=61700
